...
import {
	getLaunchAgentLabel,
	inspectLaunchAgent as inspectInstalledLaunchAgent,
} from "./launchagent.js";
...
import {
	...
	runAutopilotCoordinator,
} from "./service/autopilot.js";
import {
	buildInboxAutopilotReport,
	prepareInboxAutopilotGroup,
} from "./service/inbox-autopilot.js";
import {
...
import {
	getMeetingPrepPacketDetail,
	prepareMeetingPrepPacket,
} from "./service/meeting-prep.js";
import {
...
import {
	applyReviewApprovalFlowPayloads,
	buildReviewApprovalFlowSummary,
} from "./service/review-approval-flow.js";
import {
...
import {
	...
	type AssistantActionQueueReport,
	type AssistantActionRunResult,
	type AttentionItem,
	...
	type CalendarSource,
	type CalendarStatusReport,
	CalendarSyncState,
	type CalendarTaskScheduleResult,
	type ClientIdentity,
	type Config,
	type DoctorCheck,
	type DoctorReport,
	...
	type Paths,
	type PlanningAutopilotBundle,
	type PlanningAutopilotReport,
	...
	type PlanningRecommendationBacklogFilters,
	type PlanningRecommendationBacklogGroupReport,
	type PlanningRecommendationBacklogReport,
	type PlanningRecommendationCloseReasonCode,
	type PlanningRecommendationClosureBreakdown,
	type PlanningRecommendationClosureReport,
	type PlanningRecommendationClosureSignal,
	type PlanningRecommendationDetail,
	PlanningRecommendationFilter,
	type PlanningRecommendationFirstActionType,
	type PlanningRecommendationFollowThroughState,
	type PlanningRecommendationGroup,
	type PlanningRecommendationGroupDetail,
	type PlanningRecommendationGroupKind,
	type PlanningRecommendationHygieneFamilyReport,
	type PlanningRecommendationHygieneFilters,
	type PlanningRecommendationHygieneReport,
	type PlanningRecommendationHygieneReviewDecision,
	type PlanningRecommendationKind,
	type PlanningRecommendationOutcomeState,
	type PlanningRecommendationPolicyBacklogItem,
	type PlanningRecommendationPolicyExitItem,
	type PlanningRecommendationPolicyHistoryFamilyItem,
	type PlanningRecommendationPolicyHistoryItem,
	type PlanningRecommendationPolicyPruneResult,
	type PlanningRecommendationPolicyReport,
	type PlanningRecommendationPolicyRetentionItem,
	type PlanningRecommendationRecommendedAction,
	type PlanningRecommendationSlotState,
	type PlanningRecommendationSource,
	type PlanningRecommendationStatus,
	type PlanningRecommendationSummaryReport,
	type PlanningRecommendationTuningFamilyReport,
	type PlanningRecommendationTuningHistoryReport,
	type PlanningRecommendationTuningReport,
	type Policy,
	type PolicySnapshot,
	type RelatedDriveDoc,
	type RelatedDriveFile,
	type ReviewCalibrationReport,
	type ReviewCalibrationTarget,
	type ReviewCalibrationTargetsReport,
	type ReviewDetail,
	type ReviewFeedbackReason,
	type ReviewImpactReport,
	type ReviewNotificationDecision,
	type ReviewNotificationKind,
	type ReviewPackage,
	type ReviewPackageReport,
	type ReviewPackageSurface,
	type ReviewReport,
	type ReviewTrendsReport,
	type ReviewTuningProposal,
	type ReviewTuningReport,
...
type AssistantSafeAuditCategory =
	| "sync"
	| "task"
	| "task_suggestion"
	| "planning";
...
interface PlanningRecommendationTuningSelection
	extends PlanningRecommendationProposalSelection {
...
export class PersonalOpsService {
	...
	private readonly worklistReportInFlight = new Map<
		string,
		Promise<WorklistReport>
	>();
	...
	private readonly inboxAutopilotInFlight = new Map<
		string,
		Promise<InboxAutopilotReport>
	>();
	private readonly outboundAutopilotInFlight = new Map<
		string,
		Promise<OutboundAutopilotReport>
	>();
	private readonly planningAutopilotInFlight = new Map<
		string,
		Promise<PlanningAutopilotReport>
	>();
	private readonly assistantQueueInFlight = new Map<
		string,
		...
		return {
			...
			calendar: {
				...
				sync_status: this.config.gmailAccountEmail
					? (this.db.getCalendarSyncState(this.config.gmailAccountEmail)
							?.status ?? "idle")
					: "idle",
			},
	...
	async getStatusReport(options: {
		...
		try {
			const report = await buildStatusReport(this, {
				...options,
				skipDerived:
					Boolean(options.skipDerived) || this.reviewReadModelRefreshDepth > 0,
			});
			if (nestedWorkspaceHomeRead) {
				return report;
			}
			const [assistantQueue, nowNextWorkflow] = await Promise.all([
				this.getAssistantActionQueueReport({
			...
			const calibratedReviewApprovalFlow = trackReviewApprovalFlowCalibration(
				this,
				{
			...
			return applySurfacedNoiseReduction({ status: withReviewApprovalFlow })
				.status;
		} finally {
			this.statusWorkspaceHomeDepth = Math.max(
				0,
				this.statusWorkspaceHomeDepth - 1,
			);
		}
	...
	async getAutopilotStatusReport(
		options: {
			httpReachable: boolean;
			triggerWarm?: AutopilotTrigger | null;
		} = { httpReachable: true },
	...
	private startReviewReadModelRefresh(trigger: string): Promise<void> {
		...
		this.reviewReadModelRefreshInFlight = (async () => {
			...
			try {
				...
				this.reviewReadModelRefreshDepth = Math.max(
					0,
					this.reviewReadModelRefreshDepth - 1,
				);
	...
	async ensureReviewReadModel(options: {
		trigger: string;
		wait_for_fresh?: boolean;
		force?: boolean;
	}): Promise<void> {
	...
	async refreshReviewReadModel(trigger = "manual"): Promise<void> {
		await this.ensureReviewReadModel({
			trigger,
			wait_for_fresh: true,
			force: true,
		});
	...
	async getReviewPackage(packageId: string): Promise<ReviewPackage> {
		await this.ensureReviewReadModel({
			trigger: "review_package_detail_read",
			wait_for_fresh: true,
		});
	...
	async approveReviewTuningProposal(
		identity: ClientIdentity,
		proposalId: string,
		note: string,
	...
	async dismissReviewTuningProposal(
		identity: ClientIdentity,
		proposalId: string,
		note: string,
	): Promise<ReviewTuningProposal> {
	...
	async runAutopilot(
		...
		if (this.autopilotRunInFlight) {
			this.queuedAutopilotRequest = this.mergeAutopilotRequests(
				this.queuedAutopilotRequest,
				request,
			);
		...
		this.autopilotRunInFlight = (async () => {
			...
			let latest = await this.getAutopilotStatusReport({
				httpReachable: request.httpReachable,
			});
			while (current) {
	...
	async getNowNextWorkflowReport(options: { httpReachable: boolean }) {
		return this.dedupeInFlight(
			this.nowNextWorkflowInFlight,
			String(options.httpReachable),
	...
	async getInboxAutopilotReport(options: {
		httpReachable: boolean;
	}): Promise<InboxAutopilotReport> {
		return this.dedupeInFlight(
			this.inboxAutopilotInFlight,
			String(options.httpReachable),
			() => buildInboxAutopilotReport(this, options),
		);
	}

	async getOutboundAutopilotReport(options: {
		httpReachable: boolean;
	}): Promise<OutboundAutopilotReport> {
		return this.dedupeInFlight(
			this.outboundAutopilotInFlight,
			String(options.httpReachable),
			() => buildOutboundAutopilotReport(this, options),
		);
	}

	async getOutboundAutopilotGroup(
		groupId: string,
	): Promise<OutboundAutopilotGroup> {
		return getOutboundAutopilotGroupDetail(this, groupId);
	}

	async getPlanningAutopilotReport(options: {
		httpReachable: boolean;
	}): Promise<PlanningAutopilotReport> {
		return this.dedupeInFlight(
			this.planningAutopilotInFlight,
			String(options.httpReachable),
			() => buildPlanningAutopilotReport(this, options),
		);
	}

	async getPlanningAutopilotBundle(
		bundleId: string,
	): Promise<PlanningAutopilotBundle> {
		return getPlanningAutopilotBundleDetail(this, bundleId);
	}

	async getPrepMeetingsWorkflowReport(options: {
		httpReachable: boolean;
		scope: "today" | "next_24h";
	}) {
	...
	async requestApprovalForOutboundGroup(
		identity: ClientIdentity,
		groupId: string,
		note: string,
	...
	async applyPlanningAutopilotBundle(
		identity: ClientIdentity,
		bundleId: string,
		note: string,
		confirmed: boolean,
	) {
		return applyPlanningAutopilotBundle(
			this,
			identity,
			bundleId,
			note,
	...
	async getAssistantActionQueueReport(options: {
		httpReachable: boolean;
	}): Promise<AssistantActionQueueReport> {
		return this.dedupeInFlight(
			this.assistantQueueInFlight,
			String(options.httpReachable),
	...
	async runTrackedAssistantAction<T>(
		actionId: string,
		run: () => Promise<T>,
	): Promise<T> {
	...
	getCalendarStatusReport(): CalendarStatusReport {
		...
		return {
			...
			provider: this.config.calendarProvider,
			sync,
			calendars_synced_count: account
				? this.db.countCalendarSources(account)
				: 0,
			events_synced_count: account ? this.db.countCalendarEvents(account) : 0,
			owned_writable_calendar_count: account
				? this.db.countOwnedCalendarSources(account)
				: 0,
			personal_ops_active_event_count: account
				? this.db.countPersonalOpsCalendarEvents(account)
				: 0,
	...
	getGithubStatusReport(): GithubStatusReport {
		...
		const reviewRequestedCount = pulls.filter(
			(pull) => pull.attention_kind === "github_review_requested",
		).length;
		...
		return {
			...
			authenticated: Boolean(
				account &&
					this.dependencies.getKeychainSecret(
						this.config.githubKeychainService,
						account.keychain_account,
					),
			),
			sync_status: this.config.githubEnabled
	...
	getDriveStatusReport(): DriveStatusReport {
		...
		const report: DriveStatusReport = {
			...
			authenticated,
			sync_status: this.config.driveEnabled
				? (sync?.status ?? "not_configured")
				: "not_configured",
			last_synced_at: sync?.last_synced_at ?? null,
	...
	listGithubReviews(): GithubPullRequest[] {
		return this.db.listGithubPullRequests({
	...
	async loginGithubPat(
		...
		const account = await this.dependencies.verifyGithubToken(
			normalizedToken,
			this.config.githubKeychainService,
		);
		const previous = this.db.getGithubAccount();
		this.dependencies.setKeychainSecret(
			this.config.githubKeychainService,
			account.keychain_account,
			normalizedToken,
		);
		...
		if (previous && previous.keychain_account !== account.keychain_account) {
			this.dependencies.deleteKeychainSecret(
				previous.keychain_service,
				previous.keychain_account,
			);
		}
	...
	logoutGithub(identity: ClientIdentity): {
		cleared: boolean;
		login: string | null;
	} {
	...
	async syncMailboxMetadata(
		identity: ClientIdentity,
	): Promise<InboxStatusReport> {
	...
	async syncCalendarMetadata(
		identity: ClientIdentity,
	): Promise<CalendarStatusReport> {
	...
	async createCalendarEvent(
		...
		const { startAt, endAt } = this.assertTimedCalendarRange(
			input.start_at,
			input.end_at,
		);
		this.db.registerClient(identity);
		const { stored, target } = await this.loadCalendarWriteContext(
			input.calendar_id,
		);
		const created = await this.dependencies.createGoogleCalendarEvent(
			stored.tokensJson,
			stored.clientConfig,
			target.calendar_id,
			{
		...
		const updated = this.db.upsertCalendarEvent(
			this.toLocalCalendarEvent(
				...
				updatedProvider,
				{
					...
					last_write_by_client: identity.client_id,
					source_task_id:
						current.source_task_id ?? updatedProvider.source_task_id,
				},
			),
		);
		this.db.recordAuditEvent({
	...
	async cancelCalendarEvent(
		...
		if (!current.created_by_personal_ops) {
			throw new Error(
				"Only personal-ops created calendar events can be canceled from this command.",
			);
		}
		this.assertCalendarEventMutable(current);
		this.db.registerClient(identity);
		...
		if (current.source_task_id) {
			this.db.updateTask(current.source_task_id, {
	...
	async scheduleTaskOnCalendar(
		...
		if (!["pending", "in_progress"].includes(task.state)) {
			throw new Error(
				`Task ${taskId} cannot be scheduled from state ${task.state}.`,
			);
		}
	...
	async unscheduleTaskFromCalendar(
		identity: ClientIdentity,
		taskId: string,
		note: string,
	): Promise<CalendarTaskScheduleResult> {
		...
		if (!task.scheduled_calendar_event_id) {
			throw new Error(
				`Task ${taskId} does not have a scheduled calendar event.`,
			);
		}
		...
		if (!event) {
			...
			throw new Error(
				`Task ${taskId} linked event ${task.scheduled_calendar_event_id} is missing locally.`,
			);
		}
		if (!event.created_by_personal_ops) {
			throw new Error(
				"Only personal-ops created task events can be unscheduled from this command.",
			);
		}
	...
	private async performMailboxSync(
		identity: ClientIdentity,
	): Promise<InboxStatusReport> {
		this.assertOperatorOnly(identity, "sync mailbox metadata");
		this.db.registerClient(identity);
		const stored = await this.dependencies.loadStoredGmailTokens(
			this.config,
			this.db,
		);
		this.assertStoredMailboxMatches(stored.email);
		await this.dependencies.verifyGmailMetadataAccess(
			stored.tokensJson,
			stored.clientConfig,
		);
		...
		try {
			...
			if (
				...
				stats = await this.seedMailboxMetadata(
					stored.email,
					stored.tokensJson,
					stored.clientConfig,
				);
				...
				try {
					...
					if (
						!this.isHistoryInvalidError(error) &&
						!/History id is stale/i.test(
			...
			const profile = await this.dependencies.getGmailProfile(
				stored.tokensJson,
				stored.clientConfig,
			);
			...
			this.db.upsertMailSyncState(stored.email, "gmail", {
				...
				last_history_id:
					...
					(profile.profile.historyId
						? String(profile.profile.historyId)
						: (priorSyncState?.last_history_id ?? null)),
			...
			this.db.recordAuditEvent({
				...
				metadata: {
					...
					sync_result: {
						...
						history_id:
							stats.history_id ?? persistedSyncState?.last_history_id ?? null,
					},
					stats: this.getInboxStatusReport(),
				},
			});
			this.refreshPlanningRecommendationsInternal(
				this.systemPlanningIdentity("mailbox-sync"),
			);
			...
			const code =
				this.isHistoryInvalidError(error) || /History/i.test(message)
					? "history_invalid"
					: "sync_error";
			this.db.upsertMailSyncState(stored.email, "gmail", {
				status: "degraded",
	...
	private async performCalendarSync(
		...
		this.assertStoredMailboxMatches(stored.email);
		await this.dependencies.verifyGoogleCalendarAccess(
			stored.tokensJson,
			stored.clientConfig,
		);
		const priorSyncState = this.db.getCalendarSyncState(stored.email);
		const syncStartedAt = Date.now();
		this.db.upsertCalendarSyncState(
			stored.email,
			this.config.calendarProvider,
			{
				status: "syncing",
				last_error_code: null,
				last_error_message: null,
		...
		try {
			const stats = await this.refreshCalendarWindow(
				stored.email,
				stored.tokensJson,
				stored.clientConfig,
			);
			...
			this.refreshPlanningRecommendationsInternal(
				this.systemPlanningIdentity("calendar-sync"),
			);
			return this.getCalendarStatusReport();
		} catch (error) {
			const message =
	...
	private async performGithubSync(
		identity: ClientIdentity,
	): Promise<GithubStatusReport> {
		...
		const token = this.dependencies.getKeychainSecret(
			this.config.githubKeychainService,
			account.keychain_account,
		);
		if (!token) {
			throw new Error(
				"GitHub is enabled, but no Keychain token was found. Run `personal-ops auth github login`.",
			);
		}
		const syncStartedAt = Date.now();
		this.db.upsertGithubSyncState({
	...
	private async performDriveSync(
		identity: ClientIdentity,
		...
		if (!mailAccount) {
			throw new Error(
				"Drive is enabled, but no Google mailbox is connected. Run `personal-ops auth google login`.",
			);
		}
		if (
			this.config.includedDriveFolders.length === 0 &&
			this.config.includedDriveFiles.length === 0
		) {
			throw new Error(
		...
		try {
			await this.dependencies.verifyGoogleDriveAccess(
				stored.tokensJson,
				stored.clientConfig,
			);
			const synced = await this.dependencies.syncDriveScope(
				stored.tokensJson,
				stored.clientConfig,
				this.config,
			);
	...
	listRecentDriveFiles(limit = 10): RelatedDriveFile[] {
		...
		return files
			.map((file) =>
				this.buildRelatedDriveFile(file.file_id, "recent_file_fallback"),
			)
	...
	getRelatedFilesForTarget(
		...
		const explicit = sourceType
			? this.listExplicitRelatedFiles(sourceType, targetId)
			: [];
		const sharedParent = sourceType
			? this.listSharedParentRelatedFiles(
					sourceType,
					targetId,
					explicit,
					maxItems,
		...
		const fallback = this.listRecentDriveFiles(
			options.fallbackLimit ?? this.config.driveRecentDocsLimit,
		);
	...
	getRelatedDocsForTarget(
		...
		return this.getRelatedFilesForTarget(targetType, targetId, relatedOptions)
			...
			.map((file) => ({
				...
				match_type:
					file.match_type === "recent_file_fallback"
						? "recent_doc_fallback"
						: file.match_type,
	...
	private mapTargetTypeToDriveSourceType(
		targetType: string,
	): "calendar_event" | "task" | "draft" | null {
	...
	private listExplicitRelatedDocs(
	...
	private listExplicitRelatedFiles(
		sourceType: "calendar_event" | "task" | "draft",
		sourceId: string,
	): RelatedDriveFile[] {
		...
		for (const match of this.db.listDriveLinkProvenance(sourceType, sourceId)) {
			...
			const related = this.buildRelatedDriveFile(
				match.file_id,
				"explicit_link",
				match.source_type,
				match.source_id,
	...
	private listSharedParentRelatedFiles(
		...
		return this.db
			...
			.sort((left, right) => {
				const leftPriority =
					isGoogleDocMimeType(left.mime_type) ||
					isGoogleSheetMimeType(left.mime_type)
						? 0
						: 1;
				const rightPriority =
					isGoogleDocMimeType(right.mime_type) ||
					isGoogleSheetMimeType(right.mime_type)
						? 0
						: 1;
				...
				const leftTime = Date.parse(
					left.drive_modified_time ?? left.updated_at,
				);
				const rightTime = Date.parse(
					right.drive_modified_time ?? right.updated_at,
				);
				return rightTime - leftTime || left.name.localeCompare(right.name);
			})
			.slice(0, Math.max(0, maxItems))
			.map((file) =>
				this.buildRelatedDriveFile(
					file.file_id,
	...
	private buildRelatedDriveFile(
		...
		return {
			...
			web_view_link:
				doc?.web_view_link ?? sheet?.web_view_link ?? file?.web_view_link,
	...
	private refreshDriveLinkProvenance(): void {
		...
		const knownIds = new Set(
			this.db.listDriveFiles().map((file) => file.file_id),
		);
		...
		const mailbox =
			(this.config.gmailAccountEmail || this.db.getMailAccount()?.email) ??
			null;
		const calendarEvents = mailbox
			? this.db.listCalendarEvents({ account: mailbox, limit: 500 })
			: [];
		...
		for (const draft of this.db.listDraftArtifacts()) {
			const combined = [draft.body_text, this.stripHtml(draft.body_html)]
				.filter(Boolean)
				.join("\n");
	...
	private extractDriveMatchesFromText(
		value: string | undefined,
	): Array<{ fileId: string; url?: string }> {
		if (!value) {
			return [];
		}
	...
	listUnreadInboxThreads(limit = DEFAULT_INBOX_LIMIT): InboxThreadSummary[] {
		return this.listInboxThreadSummaries(
			...
			(summary) => summary.thread.unread_count > 0,
		);
	}
	...
	listFollowupThreads(limit = DEFAULT_INBOX_LIMIT): InboxThreadSummary[] {
		return this.listInboxThreadSummaries(
			...
			(left, right) =>
				Number(left.thread.last_message_at) -
				Number(right.thread.last_message_at),
		);
	}
	...
	enableSendWindow(
		...
		if (activeWindow) {
			throw new Error(
				`Send window ${activeWindow.window_id} is already active until ${activeWindow.expires_at}.`,
			);
		}
		this.db.registerClient(identity);
		const expiresAt = new Date(
			Date.now() + normalizedMinutes * 60 * 1000,
		).toISOString();
	...
	listTasks(
		options: {
			state?: TaskItem["state"] | undefined;
	...
	updateTask(
		...
		if (!["pending", "in_progress"].includes(task.state)) {
			throw new Error(
				`Task ${taskId} cannot be updated from state ${task.state}.`,
			);
		}
		this.db.registerClient(identity);
		const updated = this.db.updateTask(taskId, {
			...
			notes:
				updates.notes === undefined ? undefined : updates.notes?.trim() || null,
			kind: updates.kind,
			priority: updates.priority,
			owner: updates.owner,
			due_at: updates.due_at === undefined ? undefined : updates.due_at,
			remind_at:
				updates.remind_at === undefined ? undefined : updates.remind_at,
		});
		this.db.recordAuditEvent({
			client_id: identity.client_id,
			action: "task_update",
	...
	startTask(identity: ClientIdentity, taskId: string): TaskItem {
		...
		if (task.state !== "pending") {
			throw new Error(
				`Task ${taskId} cannot be started from state ${task.state}.`,
			);
		}
		this.db.registerClient(identity);
		const updated = this.db.updateTask(taskId, {
	...
	completeTask(
		...
		if (!["pending", "in_progress"].includes(task.state)) {
			throw new Error(
				`Task ${taskId} cannot be completed from state ${task.state}.`,
			);
		}
		this.db.registerClient(identity);
		const updated = this.db.updateTask(taskId, {
	...
	cancelTask(identity: ClientIdentity, taskId: string, note: string): TaskItem {
		...
		if (!["pending", "in_progress"].includes(task.state)) {
			throw new Error(
				`Task ${taskId} cannot be canceled from state ${task.state}.`,
			);
		}
		this.db.registerClient(identity);
		const updated = this.db.updateTask(taskId, {
	...
	snoozeTask(
		...
		if (!["pending", "in_progress"].includes(task.state)) {
			throw new Error(
				`Task ${taskId} cannot be snoozed from state ${task.state}.`,
			);
		}
		const updated = this.db.updateTask(taskId, {
			remind_at: until,
	...
	listTaskSuggestions(
		options: {
			status?: TaskSuggestion["status"] | undefined;
		...
		return options.include_resolved
			? this.db.listTaskSuggestions()
			: this.db.listTaskSuggestions({ status: "pending" });
	}
	...
	getTaskSuggestionDetail(suggestionId: string): TaskSuggestionDetail {
		...
		return {
			suggestion,
			accepted_task: suggestion.accepted_task_id
				? (this.db.getTask(suggestion.accepted_task_id) ?? undefined)
				: undefined,
			related_audit_events: this.getRelatedAuditEvents(
				suggestionId,
				suggestion.accepted_task_id,
			),
		};
	}
	...
	acceptTaskSuggestion(
		...
		if (suggestion.status !== "pending") {
			throw new Error(
				`Task suggestion ${suggestionId} cannot be accepted from status ${suggestion.status}.`,
			);
		}
		this.db.registerClient(identity);
		const task = this.db.createTask(identity, {
	...
	rejectTaskSuggestion(
		...
		if (suggestion.status !== "pending") {
			throw new Error(
				`Task suggestion ${suggestionId} cannot be rejected from status ${suggestion.status}.`,
			);
		}
		this.db.registerClient(identity);
		this.db.updateTaskSuggestion(suggestionId, {
	...
	listPlanningRecommendationGroups(
		...
		return this.groupPlanningRecommendations(
			this.listPlanningRecommendations(options),
		);
	}

	getPlanningRecommendationGroupDetail(
		groupKey: string,
		...
		if (recommendations.length === 0) {
			throw new Error(
				`Planning recommendation group ${groupKey} was not found.`,
			);
		}
		return this.buildPlanningRecommendationGroupDetail(
			groupKey,
			recommendations,
		);
	}
	...
	getNextPlanningRecommendationDetail(
		...
		const recommendations = this.db
			...
			.filter(
				(recommendation) => !groupKey || recommendation.group_key === groupKey,
			)
			.sort((left, right) =>
				this.compareNextActionableRecommendations(left, right),
			);
		const next = recommendations[0];
		return next
			? this.getPlanningRecommendationDetail(next.recommendation_id, options)
			: null;
	}
	...
	getPlanningRecommendationBacklogReport(
		...
		return this.computePlanningAnalytics(
	...
	getPlanningRecommendationClosureReport(
		filters:
			| number
			| PlanningRecommendationClosureFilterInput = PLANNING_CLOSED_RECENT_DAYS_LONG,
	...
	getPlanningRecommendationPolicyReport(
		identity: ClientIdentity,
	...
	reviewPlanningRecommendationHygiene(
		...
		if (
			!["review_externalized_workflow", "review_source_suppression"].includes(
				family.recommended_action,
			)
		) {
			throw new Error(
				"Only current planning hygiene candidates can be reviewed.",
			);
		}
		this.db.recordAuditEvent({
			client_id: identity.client_id,
		...
		if (!updated) {
			throw new Error(
				"The reviewed planning hygiene family could not be reloaded.",
			);
		}
		return updated;
	}
	...
	recordPlanningRecommendationHygieneProposal(
		...
		return this.upsertPlanningRecommendationHygieneProposal(
			identity,
			input,
			"proposed",
		);
	...
	archivePlanningRecommendationPolicy(
		...
		return this.recordPlanningRecommendationPolicyGovernance(
			identity,
			input,
			"policy_archived",
		);
	...
	prunePlanningRecommendationPolicyHistory(
		...
		const cutoffIso = new Date(
			Date.now() - input.older_than_days * 24 * 60 * 60 * 1000,
		).toISOString();
		...
		const candidates = this.db
			...
			.sort((left, right) =>
				this.comparePlanningPolicyRetentionItems(left, right),
			);
		const dryRun = Boolean(input.dry_run);
		const prunedCount = dryRun
			? 0
			: this.db.prunePlanningHygienePolicyGovernanceEventsBefore(
					cutoffIso,
					eventTypes,
				);
		this.db.registerClient(identity);
	...
	private upsertPlanningRecommendationHygieneProposal(
		...
		if (!this.isPlanningHygieneCandidateAction(family.recommended_action)) {
			throw new Error(
				"Only current planning hygiene candidates can have policy proposals.",
			);
		}
		if (!family.last_review_at || family.review_needed) {
			throw new Error(
				"A current operator hygiene review is required before updating a policy proposal.",
			);
		}
		const proposalType = this.derivePlanningHygieneProposalType(
			family.recommended_action,
		);
		if (!proposalType) {
			throw new Error(
				"The requested planning hygiene family does not map to a proposal type.",
			);
		}
		...
		if (!updated) {
			throw new Error(
				"The updated planning hygiene family could not be reloaded.",
			);
		}
		return updated;
	}
	...
	private recordPlanningRecommendationPolicyGovernance(
		...
		if (eventType === "policy_superseded") {
			this.assertRequiredNote(
				input.note ?? "",
				"supersede this planning policy",
			);
		}
		const familyStats = this.listPlanningRecommendationFamilyStats();
		...
		if (family.open_count > 0) {
			throw new Error(
				"Only inactive planning policy families can be archived or superseded.",
			);
		}
		const proposal = this.db.getPlanningHygienePolicyProposal(
			input.group,
			input.kind,
			input.source,
		);
		if (!proposal) {
			throw new Error(
				"An explicit planning hygiene proposal is required before policy governance can be updated.",
			);
		}
		const governanceEvent = this.db.createPlanningHygienePolicyGovernanceEvent(
			identity,
			{
				proposal_id: proposal.proposal_id,
				group_key: family.group_key,
				...
				basis_signal_updated_at:
					family.signal_updated_at ?? proposal.basis_signal_updated_at ?? null,
		...
		this.db.recordAuditEvent({
			...
			metadata: {
				...
				follow_through_state_snapshot:
					governanceEvent.follow_through_state_snapshot,
	...
	getPlanningRecommendationDetail(
		...
		if (!recommendation) {
			throw new Error(
				`Planning recommendation ${recommendationId} was not found.`,
			);
		}
		const appliedTask = recommendation.applied_task_id
			? (this.db.getTask(recommendation.applied_task_id) ?? undefined)
			: undefined;
		const detail = {
			recommendation,
			task: recommendation.source_task_id
				? (this.db.getTask(recommendation.source_task_id) ?? undefined)
				: undefined,
			thread: recommendation.source_thread_id
				? (this.db.getMailThread(recommendation.source_thread_id) ?? undefined)
				: undefined,
			...
			applied_event: recommendation.applied_calendar_event_id
				? (this.db.getCalendarEvent(recommendation.applied_calendar_event_id) ??
					undefined)
				: undefined,
			...
			source_resolved_since_created:
				recommendation.outcome_state === "source_resolved",
	...
	createPlanningRecommendation(
		...
		if (input.kind !== "schedule_task_block") {
			throw new Error(
				"Assistants may only create schedule_task_block recommendations in Phase 9.",
			);
		}
		...
		if (!["pending", "in_progress"].includes(task.state)) {
			throw new Error(
				`Task ${task.task_id} cannot be recommended from state ${task.state}.`,
			);
		}
		if (task.scheduled_calendar_event_id) {
			throw new Error(
				`Task ${task.task_id} already has a scheduled calendar event.`,
			);
		}
		const { startAt, endAt } = this.assertTimedCalendarRange(
			input.start_at,
		...
		if (!this.isTimeRangeUsable(startAt, endAt, undefined)) {
			throw new Error(
				"The proposed planning window conflicts with the current shared calendar state.",
			);
		}
		this.db.registerClient(identity);
		const candidate: PlanningRecommendationCandidate = {
			...
			dedupe_key: this.makeRecommendationDedupeKey("schedule_task_block", {
				task_id: task.task_id,
			}),
		...
		return this.getPlanningRecommendationDetail(
			recommendation.recommendation_id,
		);
	}

	replanPlanningRecommendation(
		identity: ClientIdentity,
		recommendationId: string,
		note: string,
	): PlanningRecommendationDetail {
		...
		const replanned = this.replanRecommendation(
			identity,
			recommendation,
			note.trim(),
		);
	...
	async applyPlanningRecommendation(
		...
		if (recommendation.kind === "schedule_task_block") {
			await this.applyTaskBlockRecommendation(
				identity,
				recommendation,
				note.trim(),
			);
		} else if (recommendation.kind === "schedule_thread_followup") {
			await this.applyThreadFollowupRecommendation(
				identity,
				recommendation,
				note.trim(),
			);
		} else {
			await this.applyEventPrepRecommendation(
				identity,
				recommendation,
				note.trim(),
			);
		...
		this.db.recordAuditEvent({
			...
			metadata: {
				...
				applied_calendar_event_id:
					refreshed.recommendation.applied_calendar_event_id ?? null,
				note: note.trim(),
			},
		});
		this.refreshPlanningRecommendationsInternal(identity);
	...
	rejectPlanningRecommendation(
		...
		const normalizedReasonCode =
			this.normalizePlanningDecisionReasonCode(reasonCode);
		this.applyPlanningRecommendationRejection(
			recommendation,
			note.trim(),
	...
	snoozePlanningRecommendation(
		...
		this.recordPlanningRecommendationFirstAction(
			recommendation,
			"snooze",
			identity,
		);
	...
	snoozePlanningRecommendationGroup(
		...
		this.assertOperatorOnly(
			identity,
			"snooze this planning recommendation group",
		);
		this.assertRequiredNote(note, "snooze");
		this.db.registerClient(identity);
		const recommendations = this.db
			...
			.filter(
				(recommendation) =>
					recommendation.group_key === groupKey &&
					recommendation.status === "pending",
			);
		if (recommendations.length === 0) {
			throw new Error(
				`Planning recommendation group ${groupKey} has no pending recommendations.`,
			);
		}
		const snoozedUntil = this.resolvePlanningSnoozeUntil(until, preset);
		...
		this.db.recordAuditEvent({
			...
			outcome: "success",
			metadata: {
				recommendation_ids: recommendations.map(
	...
	rejectPlanningRecommendationGroup(
		...
		this.assertOperatorOnly(
			identity,
			"reject this planning recommendation group",
		);
		this.assertRequiredNote(note, "reject");
		this.db.registerClient(identity);
		const normalizedReasonCode =
			this.normalizePlanningGroupRejectReasonCode(reasonCode);
		const recommendations = this.db
			.listPlanningRecommendations({ include_resolved: false })
			.filter(
				(recommendation) =>
	...
	async getWorklistReport(options: {
		httpReachable: boolean;
	}): Promise<WorklistReport> {
		return this.dedupeInFlight(
			this.worklistReportInFlight,
			String(options.httpReachable),
			async () => {
				const activeSendWindow = this.db.getActiveSendWindow();
				const checks = await this.collectDoctorChecks({
					deep: false,
					httpReachable: options.httpReachable,
				});
				...
				const recentRepairExecutions = this.db.listRepairExecutions({
					days: 30,
					limit: 100,
				});
				const repairPlan = buildRepairPlan({
					...
					recovery_rehearsal_missing:
						recoveryRehearsal.status !== "configured" ||
						!recoveryRehearsal.stamp,
					machine_state_origin: describeStateOrigin(
						provenance.status === "configured" ? provenance.provenance : null,
					),
				...
				const maintenanceCommitmentState = deriveMaintenanceCommitmentState({
					...
					repair_plan: repairPlan,
					recent_repair_executions: recentRepairExecutions,
					existing_commitments: this.db.listMaintenanceCommitments({
						limit: 100,
					}),
				});
				...
				const maintenanceConfidence = buildMaintenanceConfidenceSummary({
					...
					maintenance_commitment:
						maintenanceCommitmentState.maintenance_commitment,
					maintenance_defer_memory:
						maintenanceCommitmentState.maintenance_defer_memory,
					recent_repair_executions: recentRepairExecutions,
				});
				...
				const finalItems = maintenanceEscalation.cue
					? [
							...items,
							{
								...
								summary:
									maintenanceConfidence.eligible &&
									maintenanceConfidence.level === "high" &&
									maintenanceConfidence.summary
					...
					: items;
				const countsBySeverity = finalItems.reduce<
	...
	async runAttentionSweep(options: { httpReachable: boolean }) {
		this.normalizeRuntimeState();
		this.refreshPlanningRecommendationsInternal(
			this.systemPlanningIdentity("attention-sweep"),
		);
		...
		for (const item of notifyable) {
			const metadata = JSON.parse(item.metadata_json) as Record<
				string,
				unknown
			>;
			const stateMarker =
				typeof metadata.state_marker === "string"
					? metadata.state_marker
					: item.created_at;
			sendMacNotification(
				this.db,
				this.logger,
			...
			this.hub.post({
				source: "personal-ops",
				level: item.severity === "critical" ? "urgent" : "info",
				title: item.title,
				body: item.summary,
	...
	completeGoogleAuth(state: string, code: string) {
		return this.dependencies.completeGoogleAuth(
			this.config,
			this.db,
			this.logger,
			state,
	...
	async createDraft(
		...
		const providerDraftId = await this.dependencies.createGmailDraft(
			stored.tokensJson,
			stored.clientConfig,
			stored.email,
			input,
		);
		const draft = this.db.createDraftArtifact(
			identity,
			stored.email,
			providerDraftId,
			input,
		...
		this.hub.post({
			...
			level: "info",
			title: "Draft Ready",
			body: draft.subject || "A new Gmail draft is ready for review.",
			project: "mail",
		});
		this.logger.info("draft_created", {
	...
	async updateDraft(
		...
		const updated = this.db.updateDraftArtifact(
			artifactId,
			input,
			options.assistantMetadata,
		);
		if (!updated) {
			throw new Error(
				`Draft artifact ${artifactId} could not be updated locally.`,
			);
		}
		...
		this.logger.info("draft_updated", {
	...
	getReviewDetail(reviewId: string): ReviewDetail {
		...
		if (!draft) {
			throw new Error(
				`Draft artifact ${review.artifact_id} linked to review ${reviewId} was not found.`,
			);
		}
		return {
			review_item: review,
	...
	openReview(identity: ClientIdentity, reviewId: string) {
		...
		if (!["pending", "opened"].includes(existing.state)) {
			throw new Error(
				`Review item ${reviewId} cannot be opened from state ${existing.state}.`,
			);
		}
		const review =
			this.db.markReviewOpened(reviewId) ?? this.db.getReviewItem(reviewId);
		if (!review) {
			throw new Error(`Review item ${reviewId} was not found.`);
		}
		this.db.updateDraftLifecycle(review.artifact_id, {
			review_state: "opened",
		});
	...
	resolveReview(identity: ClientIdentity, reviewId: string, note: string) {
		...
		if (!["pending", "opened"].includes(review.state)) {
			throw new Error(
				`Review item ${reviewId} cannot be resolved from state ${review.state}.`,
			);
		}
	...
	requestApproval(
		...
		if (draft.status === "sent" || draft.status === "sending") {
			throw new Error(
				`Draft artifact ${artifactId} cannot enter approval from status ${draft.status}.`,
			);
		}
		const activeApproval = this.db.getActiveApprovalForArtifact(artifactId);
		if (activeApproval) {
			throw new Error(
				`Draft artifact ${artifactId} already has an active approval request.`,
			);
		}
		...
		this.hub.post({
			...
			body: draft.subject || "A draft is waiting for send approval.",
			project: "mail",
		});
		return approval;
	}
	...
	confirmApprovalAction(
		...
		if (identity.auth_role === "assistant") {
			throw new Error(
				"Confirmation tokens can only be issued from the operator channel.",
			);
		}
		...
		const expiresAt = new Date(
			Date.now() + CONFIRMATION_TTL_MINUTES * 60 * 1000,
		).toISOString();
		this.db.updateApprovalRequest(approvalId, {
			confirmation_digest: this.computeConfirmationDigest(
				approvalId,
				action,
	...
	rejectRequest(
		...
		const context = this.getApprovalContext(approvalId);
		if (
			!["pending", "approved", "send_failed"].includes(context.approval.state)
		) {
			throw new Error(
				`Approval request ${approvalId} cannot be rejected from state ${context.approval.state}.`,
			);
		}
	...
	reopenApproval(
		...
		if (context.approval.state !== "send_failed") {
			throw new Error(
				`Approval request ${approvalId} cannot be reopened from state ${context.approval.state}.`,
			);
		}
		this.db.updateApprovalRequest(approvalId, {
			state: "approved",
	...
	cancelApproval(
		...
		if (
			...
			throw new Error(
				`Approval request ${approvalId} cannot be canceled from state ${context.approval.state}.`,
			);
		}
		const now = new Date().toISOString();
		this.db.updateApprovalRequest(approvalId, {
	...
	private async collectDoctorChecks(
		...
		const oauthValidation = validateOAuthClientFile(
			this.config.oauthClientFile,
		);
		const oauthPermissions = validateSecretFilePermissions(
			this.config.oauthClientFile,
			"OAuth client file",
		);
		const localApiToken = validateSecretTextFile(
			this.paths.apiTokenFile,
			"Local API token",
		);
		const assistantApiToken = validateSecretTextFile(
			this.paths.assistantApiTokenFile,
			"Assistant API token",
		);
		const localApiTokenPermissions = validateSecretFilePermissions(
			this.paths.apiTokenFile,
			"Local API token",
		);
		const assistantApiTokenPermissions = validateSecretFilePermissions(
			this.paths.assistantApiTokenFile,
			"Assistant API token",
		);
		...
		const githubToken = githubAccount
			? this.dependencies.getKeychainSecret(
					this.config.githubKeychainService,
					githubAccount.keychain_account,
				)
			: null;
		...
		checks.push(
			oauthValidation.status === "configured"
				? this.passCheck(
						"oauth_client_file_valid",
						"OAuth client file validity",
						"OAuth client JSON is well-formed for Desktop OAuth.",
						"setup",
					)
				: oauthValidation.status === "missing" ||
						oauthValidation.status === "placeholder"
					? this.warnCheck(
							"oauth_client_file_valid",
							"OAuth client file validity",
							oauthValidation.message,
							"setup",
						)
					: this.failCheck(
							"oauth_client_file_valid",
		...
		checks.push(
			this.config.keychainService.trim()
				? this.passCheck(
						"keychain_service_configured",
						"Keychain service",
						`Keychain service is ${this.config.keychainService}.`,
						"setup",
					)
				: this.warnCheck(
						"keychain_service_configured",
			...
			!this.config.githubEnabled
				...
				: githubToken
					? this.passCheck(
							"github_token_present",
							"GitHub token",
							"GitHub Keychain token is present.",
							"integration",
						)
					: this.warnCheck(
							"github_token_present",
		...
		const githubStaleMinutes = Math.max(
			15,
			this.config.githubSyncIntervalMinutes * 3,
		);
		checks.push(
			!this.config.githubEnabled
				? this.passCheck(
						...
						"GitHub sync is not required while the integration is disabled.",
						"integration",
					)
				: githubSync?.status === "degraded"
					? this.warnCheck(
							"github_sync_fresh",
							"GitHub sync freshness",
							githubSync.last_error_message ??
								"GitHub sync is degraded. Run `personal-ops github sync now`.",
							"integration",
						)
					: !githubSync?.last_synced_at
						...
						: Date.now() - Date.parse(githubSync.last_synced_at) >
								githubStaleMinutes * 60_000
							? this.warnCheck(
									"github_sync_fresh",
									"GitHub sync freshness",
		...
		checks.push(
			!this.config.driveEnabled
				...
				: this.config.includedDriveFolders.length +
							this.config.includedDriveFiles.length >
						0
					? this.passCheck(
							"drive_scope_configured",
							"Drive scope",
			...
			!this.config.driveEnabled
				...
				: driveToken
					? this.passCheck(
							"drive_token_present",
							"Drive token",
							"Google token is present for Drive, Docs, and Sheets reads.",
							"integration",
						)
					: this.warnCheck(
							"drive_token_present",
		...
		const driveStaleMinutes = Math.max(
			30,
			this.config.driveSyncIntervalMinutes * 3,
		);
		checks.push(
			!this.config.driveEnabled
				? this.passCheck(
						...
						"Drive sync is not required while the integration is disabled.",
						"integration",
					)
				: driveSync?.status === "degraded"
					? this.warnCheck(
							"drive_sync_fresh",
							"Drive sync freshness",
							driveSync.last_error_message ??
								"Drive sync is degraded. Run `personal-ops drive sync now`.",
							"integration",
						)
					: !driveSync?.last_synced_at
						...
						: Date.now() - Date.parse(driveSync.last_synced_at) >
								driveStaleMinutes * 60_000
							? this.warnCheck(
									"drive_sync_fresh",
									"Drive sync freshness",
		...
		if (
			restoreProvenance.status === "configured" &&
			restoreProvenance.provenance
		) {
			...
			checks.push(
				this.passCheck(
					"state_origin_safe",
					"State origin",
					"No cross-machine restore provenance is recorded.",
					"setup",
				),
			);
		}
		...
		checks.push(
			options.httpReachable
				? this.passCheck(
						"daemon_http_reachable",
						"Daemon HTTP",
						"Daemon responded on localhost.",
						"runtime",
					)
				: this.failCheck(
						"daemon_http_reachable",
						"Daemon HTTP",
						"Daemon did not respond on localhost.",
						"runtime",
					),
		);
		checks.push(
		...
		for (const wrapper of wrapperHealth) {
			...
			checks.push(
				wrapperCheck(
					...
					label,
					wrapper.exists
						? `${label} exists at ${wrapper.wrapperPath}.`
						: wrapper.reason,
				),
			);
			checks.push(
			...
			checks.push(
				wrapperCheck(
					wrapper.targetFile &&
						wrapper.targetExists &&
						wrapper.targetFile === wrapper.expectedTarget
						? "pass"
						: wrapper.severity,
		...
		checks.push(
			localApiToken.status === "configured"
				? this.passCheck(
						"local_api_token_exists",
						"Local API token",
						localApiToken.message,
						"runtime",
					)
				: this.failCheck(
						"local_api_token_exists",
		...
		checks.push(
			localApiToken.status === "configured"
				? this.passCheck(
						...
						"Local API token file is non-empty.",
						"runtime",
					)
		...
		checks.push(
			localApiTokenPermissions.status === "too_broad"
				? this.warnCheck(
						"local_api_token_permissions_secure",
						"Local API token permissions",
						localApiTokenPermissions.message,
						"runtime",
					)
		...
		checks.push(
			assistantApiToken.status === "configured"
				? this.passCheck(
						"assistant_api_token_exists",
						"Assistant API token",
						assistantApiToken.message,
						"runtime",
					)
				: this.failCheck(
						"assistant_api_token_exists",
		...
		checks.push(
			assistantApiToken.status === "configured"
				? this.passCheck(
						...
						"Assistant API token file is non-empty.",
						"runtime",
					)
		...
		checks.push(
			this.passCheck(
				"send_policy_gate",
				"Send policy",
				this.policy.allowSend
					? "Live sending is enabled."
					: "Live sending is disabled by policy.",
				"runtime",
			),
		);
		...
		checks.push(
			this.db.getSchemaVersion() >= CURRENT_SCHEMA_VERSION
				? this.passCheck(
						...
						"Schema version",
						`Schema version is ${this.db.getSchemaVersion()}.`,
						"runtime",
					)
				: this.failCheck(
						"schema_version_current",
		...
		checks.push(
			schemaCompatibility.compatible
				? this.passCheck(
						"schema_runtime_compatible",
						"Schema compatibility",
						schemaCompatibility.message,
						"runtime",
					)
				: this.failCheck(
						"schema_runtime_compatible",
			...
			checks.push(
				this.failCheck(
					...
					error instanceof Error
						? error.message
						: "Approval queue could not be read.",
					"runtime",
				),
			);
		}
		...
		try {
			...
			checks.push(
				this.passCheck(
					...
					"Task suggestion table is readable.",
					"runtime",
				),
			);
		} catch (error) {
			checks.push(
				this.failCheck(
					...
					error instanceof Error
						? error.message
						: "Task suggestion table could not be read.",
					"runtime",
				),
			);
		}
		checks.push(
			this.passCheck(
				"task_reminder_sweep_healthy",
				"Task reminder sweep",
				"Task reminder sweep is active.",
				"runtime",
			),
		);
		try {
			const calendarAccount = this.config.gmailAccountEmail
				? this.config.gmailAccountEmail
				: (mailAccount?.email ?? null);
			const calendarSyncState = calendarAccount
				? this.db.getCalendarSyncState(calendarAccount)
				: null;
			this.db.listCalendarSources(calendarAccount || undefined);
			this.db.listCalendarEvents(
				calendarAccount ? { account: calendarAccount, limit: 1 } : { limit: 1 },
			);
			checks.push(
				this.passCheck(
					"calendar_tables_readable",
					"Calendar tables",
					"Calendar tables are readable.",
					"runtime",
				),
			);
			const brokenTaskLinks = this.db
				...
				.filter(
					(task) =>
						...
						!this.db.getCalendarEvent(task.scheduled_calendar_event_id),
				);
			checks.push(
			...
			checks.push(
				calendarSyncState
					...
					: this.warnCheck(
							"calendar_sync_state_readable",
							"Calendar sync state",
							"No calendar sync row exists yet.",
							"runtime",
						),
			);
			if (!this.config.calendarEnabled) {
				checks.push(
					this.warnCheck(
						"calendar_enabled",
						"Calendar subsystem",
						"Calendar syncing is disabled in config.",
						"setup",
					),
				);
				...
				const availableIds = new Set(
					this.db
						.listCalendarSources(calendarAccount || undefined)
						.map((source) => source.calendar_id),
				);
				const missing = this.config.includedCalendarIds.filter(
			...
			if (!this.config.calendarEnabled) {
				...
				checks.push(
					this.warnCheck(
						...
						"Calendar writes are disabled with calendar syncing.",
						"runtime",
					),
				);
				...
				const ownedCalendars = this.db.listOwnedCalendarSources(
				...
				checks.push(
					this.failCheck(
						"calendar_sync_ready",
						"Calendar sync readiness",
						calendarSyncState.last_error_message ??
							"Calendar sync is degraded and needs a fresh sync.",
						"runtime",
					),
				);
				checks.push(
			...
			checks.push(
				this.failCheck(
					...
					error instanceof Error
						? error.message
						: "Calendar tables could not be read.",
					"runtime",
				),
			);
			checks.push(
				this.failCheck(
					...
					error instanceof Error
						? error.message
						: "Calendar sync state could not be read.",
					"runtime",
				),
			);
			checks.push(
				this.failCheck(
					...
					error instanceof Error
						? error.message
						: "Calendar selection could not be validated.",
					"setup",
				),
			);
			checks.push(
				this.failCheck(
					...
					error instanceof Error
						? error.message
						: "Calendar sync readiness could not be evaluated.",
					"runtime",
				),
			);
			checks.push(
				this.failCheck(
					...
					error instanceof Error
						? error.message
						: "Calendar write targets could not be validated.",
					"runtime",
				),
			);
			checks.push(
				this.failCheck(
					...
					error instanceof Error
						? error.message
						: "Calendar task links could not be validated.",
					"runtime",
				),
			);
		}
		try {
			const syncState = this.config.gmailAccountEmail
				? this.db.getMailSyncState(this.config.gmailAccountEmail)
				: null;
			...
			checks.push(
				this.passCheck(
					"mail_index_readable",
					"Mail index",
					"Mailbox metadata tables are readable.",
					"runtime",
				),
			);
				...
				checks.push(
					syncState
						? this.passCheck(
								...
								"Mail sync state",
								`Sync row is readable for ${syncState.mailbox}.`,
								"runtime",
							)
						: this.warnCheck(
								"mail_sync_state_readable",
								"Mail sync state",
								"No mailbox sync row exists yet.",
								"runtime",
							),
				);
				if (syncState?.last_seeded_at) {
					...
					checks.push(
						this.warnCheck(
							"mail_seed_state_valid",
							"Mail seed state",
							"Mailbox seed has not completed yet.",
							"runtime",
						),
					);
				}
					...
					checks.push(
						this.failCheck(
							...
							syncState.last_error_message ??
								"Mailbox sync is degraded and needs a fresh sync.",
							"runtime",
						),
					);
				} else {
			...
			checks.push(
				this.failCheck(
					...
					error instanceof Error
						? error.message
						: "Mailbox metadata tables could not be read.",
					"runtime",
				),
			);
			checks.push(
				this.failCheck(
					...
					error instanceof Error
						? error.message
						: "Mailbox sync state could not be read.",
					"runtime",
				),
			);
			checks.push(
				this.failCheck(
					...
					error instanceof Error
						? error.message
						: "Mailbox seed state could not be read.",
					"runtime",
				),
			);
			checks.push(
				this.failCheck(
					...
					error instanceof Error
						? error.message
						: "Mailbox history state could not be read.",
					"runtime",
				),
			);
		}
		if (activeSendWindow) {
			...
			checks.push(
				this.passCheck(
					"send_window_consistency",
					"Send window consistency",
					"No active send window is present.",
					"runtime",
				),
			);
		}
		if (
			latestSendWindow?.state === "active" &&
			Date.parse(latestSendWindow.expires_at) <= Date.now()
		) {
			checks.push(
				this.warnCheck(
		...
		if (!mailAccount) {
			...
			const keychainProbe = probeKeychainSecret(
				this.config.keychainService,
				mailAccount.email,
			);
			checks.push(
			...
			if (!mailAccount) {
				...
				try {
					...
					if (
						profileEmail &&
						(!this.config.gmailAccountEmail ||
							profileEmail === this.config.gmailAccountEmail)
					) {
						checks.push(
							this.passCheck(
								"deep_gmail_profile_matches",
				...
				try {
					...
					this.assertStoredMailboxMatches(stored.email);
					await this.dependencies.verifyGoogleCalendarWriteAccess(
						stored.tokensJson,
						stored.clientConfig,
					);
					checks.push(
				...
				if (!this.config.driveEnabled) {
					...
					try {
						...
						this.assertStoredMailboxMatches(stored.email);
						const scopes = await this.dependencies.verifyGoogleDriveScopes(
							stored.tokensJson,
							stored.clientConfig,
						);
						const missing = [
	...
	private getApprovalContext(approvalId: string): ApprovalContext {
		...
		if (!draft) {
			throw new Error(
				`Draft artifact ${approval.artifact_id} linked to approval ${approvalId} was not found.`,
			);
		}
		return { approval, draft };
	}

	private getRelatedAuditEvents(
		...targetIds: Array<string | undefined>
	): AuditEvent[] {
		const ids = new Set(
			targetIds.filter((value): value is string => Boolean(value)),
		);
		return this.listAuditEvents({ limit: 200 })
			.filter((event) => {
	...
	private computeRiskFlags(draft: DraftArtifact): ApprovalRiskFlags {
		...
		const mailboxDomain = mailbox.includes("@")
			? (mailbox.split("@").at(-1)?.toLowerCase() ?? "")
			: "";
	...
	private buildInboxThreadSummary(thread: MailThread): InboxThreadSummary {
		...
		return {
			...
			derived_kind: this.deriveInboxThreadKind(
				thread,
				messages,
				thread.mailbox,
			),
	...
	private deriveInboxThreadKind(
		...
		const latestTime = Number(
			thread.last_message_at || latestMessage.internal_date || 0,
		);
		const unreadOldCutoff =
			Date.now() - INBOX_UNREAD_WARNING_HOURS * 60 * 60 * 1000;
		...
		const hasNewerInbound = messages.some(
			(message) =>
				Number(message.internal_date) > Number(latestMessage.internal_date) &&
				!this.isMachineAuthored(message, mailbox),
		);

		if (
			thread.in_inbox &&
			thread.unread_count > 0 &&
			latestTime <= unreadOldCutoff
		) {
			return "unread_old";
		}
		if (
			thread.in_inbox &&
			!latestIsMachine &&
			(thread.unread_count > 0 || latestTime <= unreadOldCutoff)
		) {
			return "needs_reply";
		}
	...
	private getLastDirection(
		...
		return this.isMachineAuthored(latestMessage, mailbox)
			? "outbound"
			: "inbound";
	}
	...
	private normalizeSendWindowMinutes(minutes: number): number {
		...
		if (
			minutes < SEND_WINDOW_MIN_MINUTES ||
			minutes > SEND_WINDOW_MAX_MINUTES
		) {
			throw new Error(
	...
	private buildAttentionItems(
		state: ServiceState,
		activeSendWindow: SendWindow | null,
	): AttentionItem[] {
		const now = Date.now();
		...
		const activeRecommendations = this.db.listPlanningRecommendations({
			include_resolved: false,
		});
		const activeRecommendationKeys = this.buildActiveRecommendationKeySet(
		...
		if (sync?.status === "degraded") {
			items.push({
				...
				summary:
					sync.last_error_message ?? "Mailbox metadata sync needs attention.",
				target_type: "mail_sync_state",
		...
		if (calendarSync?.status === "degraded") {
			items.push({
				...
				summary:
					calendarSync.last_error_message ?? "Calendar sync needs attention.",
				target_type: "calendar_sync_state",
				...
				metadata_json: JSON.stringify({
					state_marker: calendarSync.updated_at,
					calendar_sync_state: calendarSync.status,
				}),
			});
		}

		for (const pullRequest of this.db.listGithubPullRequests({
			attention_only: true,
		})) {
			...
			const severity: AttentionSeverity =
				pullRequest.attention_kind === "github_pr_merge_ready"
					? "info"
					: "warn";
			const title =
		...
		for (const review of this.db.listReviewItems()) {
			...
			const dueAt = new Date(
				createdAt + REVIEW_WARNING_HOURS * 60 * 60 * 1000,
			).toISOString();
			const severity: AttentionSeverity =
				createdAt <= now - REVIEW_WARNING_HOURS * 60 * 60 * 1000
					? "warn"
					: "info";
			items.push({
				...
				summary: review.subject
					? `Review draft: ${review.subject}`
					: "Review draft awaiting attention",
				target_type: "review_item",
				target_id: review.review_id,
				...
				metadata_json: JSON.stringify({
					state_marker: review.created_at,
			...
			if (approval.state === "pending") {
				const dueAt = new Date(
					Date.parse(createdAt) +
						PENDING_APPROVAL_WARNING_HOURS * 60 * 60 * 1000,
				).toISOString();
				const severity: AttentionSeverity =
					Date.parse(createdAt) <=
					now - PENDING_APPROVAL_WARNING_HOURS * 60 * 60 * 1000
						? "warn"
						: "info";
				items.push({
					...
					summary: draft?.subject
						? `Approval pending: ${draft.subject}`
						: "Approval pending",
					target_type: "approval_request",
					target_id: approval.approval_id,
					...
					metadata_json: JSON.stringify({
						state_marker: approval.updated_at,
			...
			if (approval.state === "approved") {
				...
				if (expiresAt <= now + APPROVAL_EXPIRING_WARNING_MINUTES * 60 * 1000) {
					items.push({
						...
						suggested_command: `personal-ops approval show ${approval.approval_id}`,
						metadata_json: JSON.stringify({
							state_marker: approval.expires_at,
							approval_state: approval.state,
						}),
					});
				}
		...
		const topReviewNeededFamily = this.getTopPlanningReviewNeededFamily(
			planningAnalytics.hygiene.families,
		);
		if (
			planningAnalytics.summary.review_needed_count > 0 &&
			topReviewNeededFamily
		) {
			const dominatingGroup = planningAnalytics.backlog.groups.find(
				(group) => group.group_key === topReviewNeededFamily.group_key,
			);
			const stateMarker =
				topReviewNeededFamily.signal_updated_at ?? new Date().toISOString();
			items.push({
				...
				severity:
					topReviewNeededFamily.queue_share_pct >= 50 ||
					Boolean(dominatingGroup?.dominates_queue)
						? "warn"
						: "info",
				...
				due_at: stateMarker,
				sort_rank:
					this.suppressionCandidatePriority(
						topReviewNeededFamily.recommended_action,
					) *
						1000 +
		...
		const topFollowThroughFamily =
			planningAnalytics.tuning.attention_families.find(
				(family) =>
		...
		if (topFollowThroughFamily) {
			...
			items.push({
				...
				summary: this.buildPlanningFollowThroughWorklistSummary(
					topFollowThroughFamily,
				),
				target_type: "planning_recommendation_family",
				target_id: this.getPlanningRecommendationFamilyAuditTargetId(
				...
				due_at: stateMarker,
				sort_rank:
					this.planningFollowThroughAttentionPriority(
						topFollowThroughFamily.follow_through_state,
					) *
						1000 +
		...
		if (policyAttention.worklist_kind === "planning_policy_governance_needed") {
			const stateMarker =
				policyAttention.state_marker ?? new Date().toISOString();
			items.push({
				...
				summary:
					policyAttention.summary ??
					"Review the current policy attention item.",
				target_type: "planning_recommendation_family",
				target_id: this.getPlanningRecommendationFamilyAuditTargetId(
		...
		if (
			...
			items.push({
				...
				summary:
					policyAttention.summary ??
					"Review the current policy retention candidate.",
				target_type: "planning_recommendation_policy_history",
		...
		for (const recommendation of activeRecommendations) {
			const snoozedUntilMs = recommendation.snoozed_until
				? Date.parse(recommendation.snoozed_until)
				: Number.NaN;
			if (
			...
			if (recommendation.group_key) {
				const currentCount =
					rawPlanningItemsByGroup.get(recommendation.group_key) ?? 0;
				if (currentCount >= 2) {
					continue;
				}
		...
		for (const task of this.db.listTasks()) {
			...
			const remindAtMs = task.remind_at
				? Date.parse(task.remind_at)
				: Number.NaN;
			const staleThresholdMs =
				Date.parse(task.updated_at) +
				TASK_IN_PROGRESS_STALE_HOURS * 60 * 60 * 1000;
			const subject = task.title;
			const linkedEvent = task.scheduled_calendar_event_id
				? this.db.getCalendarEvent(task.scheduled_calendar_event_id)
				: null;
			const hasActiveLinkedEvent = Boolean(
				linkedEvent && linkedEvent.status !== "cancelled",
			);
			...
			if (
				...
				!activeRecommendationKeys.has(
					this.makeRecommendationDedupeKey("schedule_task_block", {
						task_id: task.task_id,
					}),
				)
			) {
			...
			if (linkedEvent && Date.parse(linkedEvent.end_at) < now) {
				items.push({
					...
					suggested_command: `personal-ops task show ${task.task_id}`,
					metadata_json: JSON.stringify({
						state_marker: linkedEvent.end_at,
						linked_event_id: linkedEvent.event_id,
					}),
				});
			}
			if (linkedEvent && this.hasCalendarConflictForEvent(linkedEvent)) {
				items.push({
					...
					metadata_json: JSON.stringify({
						state_marker: linkedEvent.updated_at,
						linked_event_id: linkedEvent.event_id,
					}),
				});
			}
			if (
				...
				if (freeMinutes < this.config.schedulePressureFreeMinutesThreshold) {
					...
					items.push({
						...
						severity:
							dueAtMs <= now + 12 * 60 * 60 * 1000 ? "critical" : "warn",
		...
		for (const suggestion of this.db.listTaskSuggestions({
			status: "pending",
		})) {
			const createdAt = Date.parse(suggestion.created_at);
			const dueAt = new Date(
				createdAt + TASK_SUGGESTION_WARN_HOURS * 60 * 60 * 1000,
			).toISOString();
			const severity: AttentionSeverity =
			...
			items.push({
				...
				metadata_json: JSON.stringify({
					state_marker: suggestion.updated_at,
		...
		for (const summary of this.listInboxThreadSummaries(MAX_INBOX_LIMIT)) {
			const latestAtIso = this.mailTimestampToIso(
				summary.thread.last_message_at,
				summary.thread.last_synced_at,
			);
			const hasPlanningRecommendation =
				(summary.derived_kind === "needs_reply" ||
					summary.derived_kind === "stale_followup") &&
				activeRecommendationKeys.has(
					this.makeRecommendationDedupeKey("schedule_thread_followup", {
			...
			if (summary.derived_kind === "unread_old") {
				items.push({
					...
					metadata_json: JSON.stringify({
						state_marker: summary.thread.last_message_at,
						thread_kind: "unread_old",
					}),
				});
			} else if (
				summary.derived_kind === "needs_reply" &&
				!hasPlanningRecommendation
			) {
				const dueAt = new Date(
					Date.parse(latestAtIso) + INBOX_UNREAD_WARNING_HOURS * 60 * 60 * 1000,
				).toISOString();
				items.push({
		...
		if (mailbox && this.config.calendarEnabled) {
			const upcomingEvents = this.listUpcomingCalendarEvents(
				1,
				MAX_INBOX_LIMIT,
			);
			for (const event of upcomingEvents) {
				...
				if (
					activeRecommendationKeys.has(
						this.makeRecommendationDedupeKey("schedule_event_prep", {
			...
			for (const conflict of upcomingConflicts) {
				items.push({
					...
					metadata_json: JSON.stringify({
						state_marker: conflict.overlap_start_at,
					}),
				});
			}
		...
		if (activeSendWindow) {
			...
			if (expiresAt <= now + SEND_WINDOW_EXPIRING_WARNING_MINUTES * 60 * 1000) {
				items.push({
					...
					metadata_json: JSON.stringify({
						state_marker: activeSendWindow.expires_at,
						send_window_state: activeSendWindow.state,
					}),
				});
			}
		}
	...
	private mergeAutopilotRequests(
		...
		if (
			!existing.requestedProfile ||
			!next.requestedProfile ||
			existing.requestedProfile !== next.requestedProfile
		) {
			return {
				trigger: next.manual
					? next.trigger
	...
	private buildActiveRecommendationKeySet(
		recommendations: PlanningRecommendation[],
	): Set<string> {
		return new Set(
			recommendations
				.filter((recommendation) =>
					["pending", "snoozed"].includes(recommendation.status),
				)
	...
	private makeThreadRecommendationFingerprint(
	...
	private groupPlanningRecommendations(
		recommendations: PlanningRecommendation[],
	): PlanningRecommendationGroup[] {
		...
		return [...grouped.entries()]
			.map(([groupKey, items]) => {
				...
				return {
					...
					group_summary: this.describeRecommendationGroup(
						this.asPlanningRecommendationGroupKind(groupKey),
						ordered.length,
					),
					pending_count: ordered.length,
					ready_count: ordered.filter((item) => item.slot_state === "ready")
						.length,
					manual_scheduling_count: ordered.filter(
			...
			.sort(
				(left, right) =>
					right.top_rank_score - left.top_rank_score ||
	...
	private buildPlanningRecommendationGroupDetail(
		...
		const countsByOutcomeState: Record<
		...
		const nextActionableRecommendation = ordered.find(
			(recommendation) => recommendation.status === "pending",
		);
		const unresolvedRecommendations = ordered.filter((recommendation) =>
			["pending", "snoozed", "applied"].includes(recommendation.status),
		);
		const oldestUnresolvedRecommendation = [...unresolvedRecommendations].sort(
			(left, right) =>
				Date.parse(left.created_at) - Date.parse(right.created_at),
		)[0];
		const stalePendingCount = unresolvedRecommendations.filter(
			(recommendation) =>
				this.isPlanningRecommendationStalePending(recommendation),
		).length;
		const staleScheduledCount = unresolvedRecommendations.filter(
			(recommendation) =>
				this.isPlanningRecommendationStaleScheduled(recommendation),
		).length;
		const resurfacedSourceCount = unresolvedRecommendations.filter(
			(recommendation) =>
				this.isPlanningRecommendationResurfaced(
					recommendation,
					recommendations,
				),
		).length;
		...
		return {
			...
			group_summary: this.describeRecommendationGroup(
				groupKind,
				Math.max(1, pendingCount || ordered.length),
			),
			recommendations: ordered,
			...
			has_manual_scheduling_members:
				countsBySlotState.needs_manual_scheduling > 0,
	...
	private computePlanningAnalytics(
		...
		const recommendations = this.db.listPlanningRecommendations({
			include_resolved: true,
		});
		const openRecommendations = recommendations.filter((recommendation) =>
			this.isPlanningRecommendationOpen(recommendation),
		);
		const normalizedBacklogFilters =
			this.normalizePlanningBacklogFilters(backlogFilters);
		const normalizedClosureFilters = this.normalizePlanningClosureFilters(
			closureFilters ?? { days },
		);
		const normalizedHygieneFilters =
			this.normalizePlanningHygieneFilters(hygieneFilters);
		...
		const activeFamilyStats = familyStats.filter(
			(family) => family.open_count > 0,
		);
		...
		const closure = this.buildPlanningClosureReport(
			generatedAt,
			normalizedClosureFilters,
			recommendations,
		);
		const hygiene = this.buildPlanningHygieneReport(
			generatedAt,
			normalizedHygieneFilters,
			activeFamilyStats,
		);
		const followThrough = this.selectPlanningFollowThroughFamilies(
			activeFamilyStats,
			familyStats,
		);
		...
		const topSuppressionCandidate = [...hygiene.families]
			.filter((family) =>
				["review_source_suppression", "review_externalized_workflow"].includes(
					family.recommended_action,
				),
			)
			.sort(
				(left, right) =>
					this.suppressionCandidatePriority(right.recommended_action) -
						this.suppressionCandidatePriority(left.recommended_action) ||
					right.queue_share_pct - left.queue_share_pct ||
					right.open_count - left.open_count ||
					right.closed_last_30d - left.closed_last_30d ||
					left.summary.localeCompare(right.summary),
			)[0];
		const topReviewNeededCandidate = this.getTopPlanningReviewNeededFamily(
			hygiene.families,
		);
		return {
			summary: {
				...
				stale_count: groups.reduce(
					(total, group) =>
						total + group.stale_pending_count + group.stale_scheduled_count,
					0,
				),
				manual_scheduling_count: openRecommendations.filter(
					(recommendation) =>
						recommendation.slot_state === "needs_manual_scheduling",
				).length,
				closed_last_7d: recommendations.filter((recommendation) =>
					this.isRecommendationClosedWithinDays(
						recommendation,
						PLANNING_CLOSED_RECENT_DAYS_SHORT,
					),
				).length,
				closed_last_30d: recommendations.filter((recommendation) =>
					this.isRecommendationClosedWithinDays(
						recommendation,
						PLANNING_CLOSED_RECENT_DAYS_LONG,
					),
				...
				most_completed_group:
					mostCompletedGroup && mostCompletedGroup.completed_count > 0
						? {
								group_kind: this.asPlanningRecommendationGroupKind(
									mostCompletedGroup.key,
								),
								completed_count: mostCompletedGroup.completed_count,
								summary: this.describeCompletedRecommendationGroup(
									this.asPlanningRecommendationGroupKind(
										mostCompletedGroup.key,
									),
				...
				top_suppression_candidate: topSuppressionCandidate
					...
					: undefined,
				review_needed_count: hygiene.families.filter(
					(family) => family.review_needed,
				).length,
				...
				top_reviewed_stale_candidate: followThrough.top_reviewed_stale
					? {
							...
							recommended_action:
								followThrough.top_reviewed_stale.recommended_action,
				...
				top_proposal_open_candidate: followThrough.top_proposal_open
					? {
							...
							recommended_action:
								followThrough.top_proposal_open.recommended_action,
				...
				top_proposal_stale_candidate: followThrough.top_proposal_stale
					? {
							...
							recommended_action:
								followThrough.top_proposal_stale.recommended_action,
			...
			backlog: {
				generated_at: generatedAt,
				total_active_count: groups.reduce(
					(total, group) => total + group.active_count,
					0,
				),
	...
	private buildPlanningBacklogGroups(
		...
		const grouped = new Map<string, PlanningRecommendation[]>();
		const filteredOpenRecommendations = openRecommendations.filter(
			(recommendation) =>
				this.matchesPlanningBacklogRecommendationFilters(
					recommendation,
					recommendations,
					filters,
				),
		...
		return [...grouped.entries()]
			.map(([groupKey, items]) => {
				...
				const queueSharePct =
					totalOpenRecommendations === 0
						? 0
						: Number(
								((ordered.length / totalOpenRecommendations) * 100).toFixed(1),
							);
				const matchingFamilyStats = familyStats.filter(
					(family) => family.group_key === groupKey,
				);
				const reviewNeededCount = matchingFamilyStats.filter(
					(family) => family.follow_through_state === "review_needed",
				).length;
				const reviewedStaleCount = matchingFamilyStats.filter(
					(family) => family.follow_through_state === "reviewed_stale",
				).length;
				const proposalOpenCount = matchingFamilyStats.filter(
					(family) => family.follow_through_state === "proposal_open",
				).length;
				const proposalStaleCount = matchingFamilyStats.filter(
					(family) => family.follow_through_state === "proposal_stale",
				).length;
				const proposalDismissedCount = matchingFamilyStats.filter(
					(family) => family.follow_through_state === "proposal_dismissed",
				).length;
				return {
					group_key: groupKey,
					group_kind: this.asPlanningRecommendationGroupKind(groupKey),
					...
					stale_pending_count: ordered.filter((recommendation) =>
						this.isPlanningRecommendationStalePending(recommendation),
					).length,
					stale_scheduled_count: ordered.filter((recommendation) =>
						this.isPlanningRecommendationStaleScheduled(recommendation),
					).length,
					manual_scheduling_count: ordered.filter(
						(recommendation) =>
					...
					median_open_age_hours: this.calculateMedianHoursSince(
						ordered.map((recommendation) => recommendation.created_at),
					),
					closed_last_30d: closureMix.closed_last_30d,
					...
					dominant_close_reason_last_30d:
						closureMix.dominant_close_reason_last_30d,
					queue_share_pct: queueSharePct,
					dominates_queue: queueSharePct >= 60,
					closure_meaning_summary:
						this.summarizeFamilyClosureMeaning(matchingFamilyStats),
	...
	private buildPlanningClosureReport(
		...
		const byKind = this.buildPlanningClosureBreakdowns(
			filteredRecommendations,
		...
		const byCloseReason = this.buildPlanningClosureBreakdowns(
			filteredRecommendations.filter(
				(recommendation) => recommendation.close_reason_code,
			),
			(recommendation) => recommendation.close_reason_code ?? "unknown",
			filters.days,
		);
		const bySource = this.buildPlanningClosureBreakdowns(
			filteredRecommendations,
		...
		return {
			...
			totals: this.buildPlanningClosureBreakdown(
				"all",
				filteredRecommendations,
				filters.days,
			),
	...
	private buildPlanningClosureBreakdown(
		...
		return {
			key,
			created_count: recommendations.filter((recommendation) =>
				this.isTimestampWithinDays(recommendation.created_at, days),
			).length,
			first_action_count: recommendations.filter((recommendation) =>
				this.isTimestampWithinDays(recommendation.first_action_at, days),
			).length,
			closed_count: recommendations.filter((recommendation) =>
				this.isTimestampWithinDays(recommendation.closed_at, days),
			).length,
			...
			median_time_to_first_action_minutes: this.calculateMedianMinutesBetween(
				recommendations
					...
					.map((recommendation) => ({
						...
						end: recommendation.first_action_at,
					})),
			),
			median_time_to_close_minutes: this.calculateMedianMinutesBetween(
				recommendations
					...
					.map((recommendation) => ({
						start: recommendation.created_at,
						end: recommendation.closed_at,
					})),
			),
			closure_meaning_summary: this.buildClosureMeaningSummary({
				closedCount: recommendations.filter((recommendation) =>
					this.isTimestampWithinDays(recommendation.closed_at, days),
				).length,
				completedCount,
	...
	private normalizePlanningClosureFilters(
		filters: number | PlanningRecommendationClosureFilterInput | undefined,
	...
	private matchesPlanningClosureFilters(
		...
		if (
			...
			recommendation.close_reason_code !== filters.close_reason
		) {
			return false;
		}
		return (
			this.isTimestampWithinDays(recommendation.created_at, filters.days) ||
			this.isTimestampWithinDays(
				recommendation.first_action_at,
				filters.days,
			) ||
	...
	private getPlanningRecommendationFamilyKey(
		recommendation: PlanningRecommendation,
	): string {
		return [
			recommendation.group_key ?? "urgent_unscheduled_tasks",
	...
	private listPlanningHygieneReviewStates(): Map<
		string,
		PlanningRecommendationHygieneReviewState
	> {
		...
		for (const event of this.listAuditEvents({
			...
			states.set(event.target_id, {
				...
				last_review_by_actor:
					typeof metadata.reviewed_by_actor === "string"
						? metadata.reviewed_by_actor
						: null,
				last_review_note:
	...
	private listPlanningHygieneProposalStates(): Map<
		string,
	...
	private isPlanningHygieneCandidateAction(
		action: PlanningRecommendationRecommendedAction,
	): boolean {
		return (
			action === "review_externalized_workflow" ||
			action === "review_source_suppression"
		);
	}
	...
	private derivePlanningFollowThroughState(input: {
		...
		if (
			input.reviewAgeDays !== null &&
			input.reviewAgeDays >= PLANNING_FOLLOW_THROUGH_STALE_DAYS
		) {
	...
	private computePlanningHygieneSignalUpdatedAt(
		recommendations: PlanningRecommendation[],
	): string | null {
	...
	private selectLatestPlanningTimestamp(
	...
	private buildPlanningHygieneReviewSummary(
		...
		const actor = reviewState.last_review_by_actor
			? ` by ${reviewState.last_review_by_actor}`
			: "";
		const note = reviewState.last_review_note
			? ` Note: ${reviewState.last_review_note}`
			: "";
		return `Reviewed ${reviewState.last_review_at}${actor}: ${reviewState.last_review_decision}.${note}`;
	}

	private asPlanningHygieneReviewDecision(
		value: unknown,
	): PlanningRecommendationHygieneReviewDecision | null {
		...
		return [...grouped.entries()]
			.map(([familyKey, rows]) => {
				...
				const recommendedAction =
					this.recommendActionForClosureSignal(closureSignal);
				const openCount = openCounts.get(familyKey) ?? 0;
				const queueSharePct =
					totalOpenRecommendations === 0
						? 0
						: Number(((openCount / totalOpenRecommendations) * 100).toFixed(1));
				const signalUpdatedAt =
					this.computePlanningHygieneSignalUpdatedAt(rows);
				const familyAuditTargetId =
					this.getPlanningRecommendationFamilyAuditTargetId(
						groupKey,
						sample.kind,
						sample.source,
				...
				const reviewNeeded =
					...
					(!reviewState ||
						!signalUpdatedAt ||
						Date.parse(reviewState.last_review_at) <
							Date.parse(signalUpdatedAt));
				const reviewAgeDays = this.calculateAgeDays(
					reviewState?.last_review_at ?? null,
				);
				const proposalAgeDays = this.calculateAgeDays(
					proposalState?.updated_at ?? null,
				);
				const proposalStale = Boolean(
					proposalState?.status === "proposed" &&
						((signalUpdatedAt &&
							...
							Date.parse(signalUpdatedAt) >
								Date.parse(proposalState.basis_signal_updated_at)) ||
							(proposalAgeDays !== null &&
								proposalAgeDays >= PLANNING_FOLLOW_THROUGH_STALE_DAYS)),
				);
				...
				return {
					...
					proposal_type:
						...
						this.derivePlanningHygieneProposalType(recommendedAction),
					...
					last_active_at: this.selectLatestPlanningTimestamp(
						rows.map(
							(recommendation) =>
								recommendation.updated_at ?? recommendation.created_at,
						),
					),
					last_closed_at: this.selectLatestPlanningTimestamp(
						rows.map((recommendation) => recommendation.closed_at),
					),
	...
	private buildPlanningHygieneReport(
		...
		const families = familyStats
			.filter((family) =>
				this.matchesPlanningHygieneFamilyFilters(family, filters),
			)
			.map(
	...
	private shapePlanningRecommendationDetail(
		...
		return {
			...detail,
			related_audit_events: detail.related_audit_events.filter(
				(event) => !hiddenActions.has(event.action),
			),
		};
	}
	...
	private shapeAuditEventForAssistant(event: AuditEvent): AuditEvent | null {
		...
		const shaped = this.buildAssistantSafeAuditShape(
			event.action,
			event.outcome,
			metadata,
		);
	...
	private parseAuditEventMetadata(
		metadataJson: string,
	...
	private listAssistantSafeAuditActionsForCategory(
		category: AssistantSafeAuditCategory,
	): string[] {
	...
	> {
		return {
				...
				return {
					...
					metadata:
						eventOutcome === "success"
							? {
									sync_result: {
										messages_refreshed: this.readAuditNumber(
											result?.messages_refreshed,
										),
										messages_deleted: this.readAuditNumber(
											result?.messages_deleted,
										),
										threads_recomputed: this.readAuditNumber(
											result?.threads_recomputed,
										),
				...
				return {
					...
					metadata:
						eventOutcome === "success"
							? {
									provider:
										typeof eventMetadata.provider === "string"
											? eventMetadata.provider
											: null,
									sync_result: {
										calendars_refreshed: this.readAuditNumber(
											result?.calendars_refreshed,
										),
										events_refreshed: this.readAuditNumber(
											result?.events_refreshed,
										),
										duration_ms: this.readAuditNumber(result?.duration_ms),
									},
								}
			...
			task_create: (_eventOutcome, eventMetadata) => ({
				...
				metadata: {
					owner:
						typeof eventMetadata.owner === "string"
							? eventMetadata.owner
							: null,
					kind:
						typeof eventMetadata.kind === "string" ? eventMetadata.kind : null,
					priority:
						typeof eventMetadata.priority === "string"
			...
			task_update: (_eventOutcome, eventMetadata) => ({
				...
				metadata: {
					changes: this.readAuditArray(eventMetadata.changes).filter(
			...
			task_snooze: (_eventOutcome, eventMetadata) => ({
				...
				metadata: {
					until:
						typeof eventMetadata.until === "string"
							? eventMetadata.until
							: null,
				},
			}),
			...
			task_suggestion_accept: (_eventOutcome, eventMetadata) => ({
				...
				metadata: {
					task_id:
						typeof eventMetadata.task_id === "string"
							? eventMetadata.task_id
							: null,
				},
			}),
			...
			planning_recommendation_create: (_eventOutcome, eventMetadata) => ({
				...
				metadata: {
					kind:
						typeof eventMetadata.kind === "string" ? eventMetadata.kind : null,
					source:
						typeof eventMetadata.source === "string"
			...
			planning_recommendation_replan: (_eventOutcome, eventMetadata) => ({
				...
				metadata: {
					kind:
						typeof eventMetadata.kind === "string" ? eventMetadata.kind : null,
					proposed_start_at:
						typeof eventMetadata.proposed_start_at === "string"
							? eventMetadata.proposed_start_at
							: null,
					proposed_end_at:
						typeof eventMetadata.proposed_end_at === "string"
							? eventMetadata.proposed_end_at
							: null,
				},
			}),
			planning_recommendation_refresh: (_eventOutcome, eventMetadata) => ({
				...
				metadata: {
					...
					superseded_count: this.readAuditNumber(
						eventMetadata.superseded_count,
					),
			...
			planning_recommendation_apply: (_eventOutcome, eventMetadata) => ({
				...
				metadata: {
					kind:
						typeof eventMetadata.kind === "string" ? eventMetadata.kind : null,
					applied_task_id:
						typeof eventMetadata.applied_task_id === "string"
							? eventMetadata.applied_task_id
							: null,
				},
			}),
			planning_recommendation_reject: (_eventOutcome, eventMetadata) => ({
				...
				metadata: {
					kind:
						typeof eventMetadata.kind === "string" ? eventMetadata.kind : null,
					reason_code:
						typeof eventMetadata.reason_code === "string"
							? eventMetadata.reason_code
							: null,
				},
			}),
				...
				metadata: {
					...
					until:
						typeof eventMetadata.until === "string"
							...
							: null,
					preset:
						typeof eventMetadata.preset === "string"
							? eventMetadata.preset
							: null,
				},
			}),
				...
				metadata: {
					...
					preset:
						typeof eventMetadata.preset === "string"
							? eventMetadata.preset
							: null,
					recommendation_count: this.readAuditArray(
						eventMetadata.recommendation_ids,
					).length,
				},
			}),
			...
			planning_recommendation_outcome_update: (
				_eventOutcome,
				eventMetadata,
			) => ({
				category: "planning",
				...
				metadata: {
					outcome_state:
						typeof eventMetadata.outcome_state === "string"
							? eventMetadata.outcome_state
							: null,
					outcome_source:
						typeof eventMetadata.outcome_source === "string"
							? eventMetadata.outcome_source
							: null,
				},
			}),
		};
	...
	private buildPlanningPolicyReport(): PlanningRecommendationPolicyReport {
		const generatedAt = new Date().toISOString();
		const familyStats = this.listPlanningRecommendationFamilyStats();
		const latestGovernanceEvents =
			this.listLatestPlanningPolicyGovernanceEvents();
		const activeBacklogFamilies = familyStats
			...
			.sort((left, right) =>
				this.comparePlanningPolicyBacklogFamilies(left, right),
			);
		const activeBacklog = activeBacklogFamilies
			.slice(0, 10)
			.map((family) => this.toPlanningPolicyBacklogItem(family));
		const allGovernanceEvents =
			this.db.listPlanningHygienePolicyGovernanceEvents();
		const allPolicyHistory = this.buildPlanningPolicyHistoryRecentEvents(
			allGovernanceEvents,
			familyStats,
		);
		const policyHistoryFamilies = this.buildPlanningPolicyHistoryFamilies(
			allGovernanceEvents,
			familyStats,
		);
		const governanceWatchlist = policyHistoryFamilies.filter(
			(item) =>
				item.family.recommended_action === "review_policy_churn" ||
				item.family.recommended_action === "prune_old_history",
		);
		const recentPolicyExits = this.buildPlanningPolicyRecentExits(
			familyStats,
			latestGovernanceEvents,
		);
		const retentionCandidates =
			this.buildPlanningPolicyRetentionCandidates(familyStats);
		const activeProposedRows = activeBacklog.filter(
			(item) => item.proposal_status === "proposed",
		);
		const activeDismissedRows = activeBacklog.filter(
			(item) => item.proposal_status === "dismissed",
		);
		const archivedRows = allPolicyHistory.filter(
			(item) => item.governance_event_type === "policy_archived",
		);
		const supersededRows = allPolicyHistory.filter(
		...
		return {
			generated_at: generatedAt,
			active_proposed_count: activeBacklogFamilies.filter(
				(family) => family.proposal_status === "proposed",
			).length,
			active_dismissed_for_now_count: familyStats.filter(
				(family) =>
					family.open_count > 0 && family.proposal_status === "dismissed",
			).length,
			...
			repeated_policy_family_count: policyHistoryFamilies.filter(
				(item) => item.family.total_governance_events >= 2,
			).length,
			mixed_outcome_policy_family_count: policyHistoryFamilies.filter(
				(item) => item.family.has_mixed_governance_outcomes,
			).length,
			...
			top_active_proposed_summary:
				activeProposedRows[0]?.summary ??
				activeBacklogFamilies.find(
					(family) => family.proposal_status === "proposed",
				)?.summary ??
				null,
			top_active_dismissed_summary:
				activeDismissedRows[0]?.summary ??
				activeBacklogFamilies.find(
					(family) => family.proposal_status === "dismissed",
				)?.summary ??
				null,
			top_archived_summary: archivedRows[0]
				? this.sanitizePlanningPolicySummary(archivedRows[0].summary)
				: null,
			top_superseded_summary: supersededRows[0]
				? this.sanitizePlanningPolicySummary(supersededRows[0].summary)
				: null,
			top_recent_policy_exit_summary:
				recentPolicyExits[0]?.exit_summary ?? null,
			top_retention_candidate_summary:
				governanceWatchlist.find(
					(item) => item.family.recommended_action === "prune_old_history",
				)?.family.summary ??
				retentionCandidates[0]?.summary ??
				null,
			top_repeated_policy_family_summary:
				policyHistoryFamilies.find(
					(item) => item.family.total_governance_events >= 2,
				)?.family.summary ?? null,
			...
			policy_history_families: policyHistoryFamilies
				.slice(0, 10)
	...
	private buildPlanningPolicyHistoryRecentEvents(
		...
		return events
			.map((event) => {
				const family = familyStats.find(
					(item) =>
						item.group_key === event.group_key &&
			...
			.filter((item): item is PlanningRecommendationPolicyHistoryItem =>
				Boolean(item),
			)
			.sort(
				(left, right) =>
					Date.parse(right.governance_recorded_at) -
						Date.parse(left.governance_recorded_at) ||
	...
	private buildPlanningPolicyHistoryFamilies(
		...
		const rawEvents = this.buildPlanningPolicyHistoryRecentEvents(
			events,
			familyStats,
		);
		const rawEventsByFamily = new Map<
			string,
			PlanningRecommendationPolicyHistoryItem[]
		>();
		for (const item of rawEvents) {
			const familyKey = this.getPlanningRecommendationFamilyAuditTargetId(
				item.group_key,
				item.kind,
				item.source,
			);
		...
		const retentionCandidates =
			this.buildPlanningPolicyRetentionCandidates(familyStats);
		const retentionByFamily = new Map<
			string,
		...
		return [...rawEventsByFamily.entries()]
			.map(([familyKey, groupedRawEvents]) => {
				...
				const groupedRetentionCandidates =
					retentionByFamily.get(familyKey) ?? [];
				return {
					family: this.toPlanningPolicyHistoryFamilyItem(
						family,
						proposal,
						groupedRawEvents,
						groupedRetentionCandidates,
					),
			...
			.filter(
				(item): item is PlanningRecommendationPolicyHistoryFamilySelection =>
					Boolean(item),
			)
			.sort((left, right) =>
				this.comparePlanningPolicyHistoryFamilyItems(left.family, right.family),
			);
	}
	...
	private buildPlanningPolicyRecentExits(
		...
		return familyStats
			...
			.filter((family) =>
				this.isTimestampWithinDays(
					family.last_closed_at ?? undefined,
					PLANNING_CLOSED_RECENT_DAYS_LONG,
				),
			)
			.filter((family) =>
				this.db.getPlanningHygienePolicyProposal(
					family.group_key,
					family.kind,
					family.source,
				),
			...
			.sort(
				(left, right) =>
					Date.parse(right.last_closed_at ?? "") -
						Date.parse(left.last_closed_at ?? "") ||
					Date.parse(right.last_active_at ?? "") -
						Date.parse(left.last_active_at ?? "") ||
					left.group_key.localeCompare(right.group_key) ||
					left.kind.localeCompare(right.kind) ||
					left.source.localeCompare(right.source),
			)
	...
	private listLatestPlanningPolicyGovernanceEvents(): Map<
		string,
	...
	private toPlanningPolicyBacklogItem(
		...
		return {
			...
			source: family.source,
			proposal_type:
				family.proposal_type ??
				this.derivePlanningHygieneProposalType(family.recommended_action)!,
			proposal_status: family.proposal_status ?? "proposed",
			follow_through_state: family.follow_through_state as
				| "proposal_open"
				| "proposal_stale"
				| "proposal_dismissed",
			open_count: family.open_count,
	...
	private toPlanningPolicyHistoryItem(
		...
		const actionLabel =
			event.event_type === "policy_archived" ? "archived" : "superseded";
		...
		return {
			...
			final_proposal_status:
				event.proposal_status_snapshot ??
				proposal?.status ??
				family.proposal_status,
			last_follow_through_state:
				event.follow_through_state_snapshot ?? family.follow_through_state,
	...
	private toPlanningPolicyRetentionItem(
		...
		const familyLabel =
			family?.summary ??
			`${this.humanizePlanningKind(event.kind)} (${event.source.replaceAll("_", " ")})`;
		return {
			governance_event_id: event.governance_event_id,
			group_key: event.group_key,
			group_kind:
				family?.group_kind ??
				this.asPlanningRecommendationGroupKind(event.group_key),
			kind: event.kind,
			source: event.source,
			governance_event_type: event.event_type,
			governance_recorded_at: event.recorded_at,
	...
	private toPlanningPolicyHistoryFamilyItem(
		...
		const recentGovernanceEvents90d = rawEvents.filter((item) =>
			this.isTimestampWithinDays(
				item.governance_recorded_at,
				PLANNING_CLOSED_RECENT_DAYS_LONG * 3,
			),
		).length;
		const highChurn = recentGovernanceEvents90d >= 2;
		const recommendedAction =
			hasMixedGovernanceOutcomes || highChurn
				? "review_policy_churn"
	...
	private buildPlanningPolicyHistoryFamilySummary(
		...
		if (hasMixedGovernanceOutcomes) {
			...
			pieces.push(
				`It has repeated governance churn with ${recentGovernanceEvents90d} events in the last 90 days.`,
			);
		} else if (retentionCandidateCount > 0) {
			pieces.push(
				`${retentionCandidateCount} older governance ${retentionCandidateCount === 1 ? "entry is" : "entries are"} ready for retention review.`,
			);
		} else {
	...
	private comparePlanningPolicyHistoryFamilyItems(
		...
		const leftRepeatedPressure =
			(left.recent_governance_events_90d >= 2 ? 1000 : 0) +
			left.recent_governance_events_90d * 10 +
			left.total_governance_events;
		...
		return (
			...
			Number(right.has_mixed_governance_outcomes) -
				Number(left.has_mixed_governance_outcomes) ||
			Date.parse(right.latest_governance_recorded_at) -
				Date.parse(left.latest_governance_recorded_at) ||
			left.group_key.localeCompare(right.group_key) ||
			left.kind.localeCompare(right.kind) ||
			left.source.localeCompare(right.source)
		);
	...
	private buildPlanningTuningReport(
		...
		return {
			...
			attention_families: selection.attention_families.map((family) =>
				this.toPlanningTuningAttentionFamily(family),
			),
			recently_closed_families: selection.recently_closed_families,
		};
	}

	private buildPlanningClosureMix(
		recommendations: PlanningRecommendation[],
	): PlanningClosureMix {
		...
		const dominantCloseReason =
			[...closeReasonCounts.entries()].sort(
				(left, right) => right[1] - left[1] || left[0].localeCompare(right[0]),
			)[0]?.[0] ?? null;
		return {
			closed_last_30d: recent.length,
			completed_last_30d: recent.filter(
				(recommendation) => recommendation.outcome_state === "completed",
			).length,
			handled_elsewhere_last_30d: recent.filter(
				(recommendation) =>
					recommendation.outcome_state === "handled_elsewhere",
			).length,
			source_resolved_last_30d: recent.filter(
				(recommendation) => recommendation.outcome_state === "source_resolved",
			).length,
	...
	private classifyPlanningClosureSignal(
		...
		if (
			closureMix.source_resolved_last_30d / closureMix.closed_last_30d >=
			0.6
		) {
			return "mostly_source_resolved";
		}
		if (
			closureMix.handled_elsewhere_last_30d / closureMix.closed_last_30d >=
			0.5
		) {
	...
	private buildClosureMeaningSummary(counts: {
		...
		if (
			counts.sourceResolvedCount > counts.handledElsewhereCount &&
			counts.sourceResolvedCount > counts.completedCount
		) {
		...
		if (
			counts.handledElsewhereCount > counts.sourceResolvedCount &&
			counts.handledElsewhereCount > counts.completedCount
		) {
			return "Recent closures mostly indicate work is leaving the queue, which points to workflow-routing or visibility tuning.";
		}
		if (
			counts.completedCount > counts.sourceResolvedCount &&
			counts.completedCount > counts.handledElsewhereCount
		) {
	...
	private buildPlanningHygieneSummary(input: {
		...
		const groupSummary = this.describeRecommendationGroup(
			input.groupKind,
			Math.max(1, input.openCount),
		);
	...
	private summarizeFamilyClosureMeaning(
	...
	private comparePlanningFollowThroughFamilies(
		...
		const leftAge =
			left.follow_through_state === "proposal_stale" ||
			left.follow_through_state === "proposal_open"
				? (left.proposal_age_days ?? -1)
				: (left.review_age_days ?? -1);
		const rightAge =
			right.follow_through_state === "proposal_stale" ||
			right.follow_through_state === "proposal_open"
	...
	private planningFollowThroughAttentionPriority(
		state: PlanningRecommendationFollowThroughState | null,
	): number {
		if (state === "proposal_stale") {
			return 4;
	...
	private buildPlanningRecentlyClosedFamilies(
		...
		return [...families]
			...
			.filter(
				(family) =>
					(family.last_review_at !== null &&
						(family.review_age_days ?? Number.POSITIVE_INFINITY) <=
							PLANNING_CLOSED_RECENT_DAYS_LONG) ||
					(family.proposal_updated_at !== null &&
						(family.proposal_age_days ?? Number.POSITIVE_INFINITY) <=
							PLANNING_CLOSED_RECENT_DAYS_LONG),
			)
	...
	private buildPlanningClosedFamilyExitSummary(
		family: PlanningRecommendationFamilyStats,
	): string {
	...
	private selectPlanningFollowThroughFamilies(
		...
		const rows = [...activeFamilies].filter(
			(family) => family.follow_through_state !== null,
		);
		...
		return {
			review_needed: rows.filter(
				(family) => family.follow_through_state === "review_needed",
			).length,
			reviewed_fresh: rows.filter(
				(family) => family.follow_through_state === "reviewed_fresh",
			).length,
			reviewed_stale: rows.filter(
				(family) => family.follow_through_state === "reviewed_stale",
			).length,
			proposal_open: rows.filter(
				(family) => family.follow_through_state === "proposal_open",
			).length,
			proposal_stale: rows.filter(
				(family) => family.follow_through_state === "proposal_stale",
			).length,
			proposal_dismissed: rows.filter(
				(family) => family.follow_through_state === "proposal_dismissed",
			).length,
			top_review_needed: byState("review_needed")[0] as
				| PlanningRecommendationFamilyStats
				| undefined,
			top_reviewed_stale: byState("reviewed_stale")[0] as
				| PlanningRecommendationFamilyStats
				| undefined,
			top_proposal_open: byState("proposal_open")[0] as
				| PlanningRecommendationFamilyStats
				| undefined,
			top_proposal_stale: byState("proposal_stale")[0] as
				| PlanningRecommendationFamilyStats
				| undefined,
			attention_families: rows
				...
				.sort((left, right) =>
					this.comparePlanningAttentionFamilies(left, right),
				)
				.slice(0, 10),
			recently_closed_families:
				this.buildPlanningRecentlyClosedFamilies(allFamilies),
		};
	}
	...
	private suppressionCandidatePriority(
		action: PlanningRecommendationRecommendedAction,
	): number {
		if (action === "review_source_suppression") {
			return 3;
	...
	private getPlanningCalibrationFamilyKey(
		recommendation: PlanningRecommendation,
	): string {
		...
		return new Map(
			[...grouped.entries()].map(([key, rows]) => [
				key,
				{
					...
					completed_count: rows.filter(
						(recommendation) => recommendation.outcome_state === "completed",
					).length,
					handled_elsewhere_count: rows.filter(
						(recommendation) =>
							recommendation.outcome_state === "handled_elsewhere",
					).length,
					source_resolved_count: rows.filter(
						(recommendation) =>
							recommendation.outcome_state === "source_resolved",
					).length,
				},
	...
	private isPlanningRecommendationOpen(
		recommendation: PlanningRecommendation,
	): boolean {
		return (
			!recommendation.closed_at &&
	...
	private isPlanningRecommendationStalePending(
		recommendation: PlanningRecommendation,
	): boolean {
		return (
			recommendation.status === "pending" &&
			this.isTimestampOlderThanHours(
				recommendation.created_at,
				PLANNING_STALE_PENDING_HOURS,
			)
		);
	}

	private isPlanningRecommendationStaleScheduled(
		recommendation: PlanningRecommendation,
	): boolean {
		if (
			recommendation.status !== "applied" ||
			recommendation.outcome_state !== "scheduled" ||
			recommendation.closed_at
		) {
			return false;
		}
		const scheduledAt =
			recommendation.outcome_recorded_at ??
			recommendation.first_action_at ??
			recommendation.resolved_at ??
			recommendation.created_at;
		return this.isTimestampOlderThanHours(
			scheduledAt,
			PLANNING_STALE_SCHEDULED_HOURS,
		);
	}
	...
	private isPlanningRecommendationResurfaced(
		...
		const cutoffMs =
			Date.now() - PLANNING_RESURFACED_LOOKBACK_DAYS * 24 * 60 * 60 * 1000;
		...
		return recommendations.some((candidate) => {
			if (
				...
				candidate.dedupe_key !== recommendation.dedupe_key
			) {
				return false;
			}
	...
	private isPlanningRecommendationOutcomeWithinDays(
		...
		return (
			...
			this.isTimestampWithinDays(
				recommendation.closed_at ?? recommendation.outcome_recorded_at,
				days,
			)
		);
	}

	private isRecommendationClosedWithinDays(
		recommendation: PlanningRecommendation,
		days: number,
	): boolean {
	...
	private isTimestampOlderThanHours(
		timestamp: string | undefined,
		hours: number,
	): boolean {
		const value = Date.parse(timestamp ?? "");
		return (
			Number.isFinite(value) && Date.now() - value >= hours * 60 * 60 * 1000
		);
	}

	private isTimestampWithinDays(
		timestamp: string | undefined,
		days: number,
	): boolean {
		const value = Date.parse(timestamp ?? "");
		return (
			Number.isFinite(value) && value >= Date.now() - days * 24 * 60 * 60 * 1000
		);
	}

	private calculateMedianHoursSince(
	...
	private calculateMedian(values: number[]): number | null {
		...
		return Number(
			(((sorted[middle - 1] ?? 0) + (sorted[middle] ?? 0)) / 2).toFixed(1),
		);
	}

	private summarizePlanningBacklog(
		report: PlanningRecommendationBacklogReport,
	): string | null {
	...
	private summarizePlanningClosure(
		report: PlanningRecommendationClosureReport,
	): string | null {
	...
	private summarizePlanningDominantBacklog(
		report: PlanningRecommendationBacklogReport,
	): string | null {
	...
	private summarizePlanningReviewNeeded(
		report: PlanningRecommendationHygieneReport,
	): string | null {
		return (
			this.getTopPlanningReviewNeededFamily(report.families)?.summary ?? null
		);
	}

	private summarizePlanningSuppressionCandidate(
		report: PlanningRecommendationHygieneReport,
	): string | null {
	...
	private summarizePlanningHygiene(
		report: PlanningRecommendationHygieneReport,
	): string | null {
		return (
			this.summarizePlanningReviewNeeded(report) ??
			this.summarizePlanningSuppressionCandidate(report)
		);
	}

	private describeCompletedRecommendationGroup(
		groupKind: PlanningRecommendationGroupKind,
		count: number,
	): string {
	...
	private asPlanningRecommendationGroupKind(
		value: string,
	): PlanningRecommendationGroupKind {
		if (
			value === "urgent_unscheduled_tasks" ||
			value === "urgent_inbox_followups" ||
			value === "near_term_meeting_prep"
		) {
			return value;
		}
	...
	private describeRecommendationGroup(
		groupKind: PlanningRecommendationGroupKind,
		count: number,
	): string {
		if (groupKind === "urgent_unscheduled_tasks") {
	...
	private calculateRecommendationRank(
		...
		if (Number.isFinite(proposedStartMs)) {
			const hoursAway = Math.max(
				0,
				(proposedStartMs - Date.now()) / (60 * 60_000),
			);
			if (hoursAway <= 2) {
				score += 40;
		...
		if (
			recommendation.kind === "schedule_task_block" &&
			recommendation.source_task_id
		) {
		...
		if (
			recommendation.kind === "schedule_event_prep" &&
			recommendation.source_calendar_event_id
		) {
			const event = this.db.getCalendarEvent(
				recommendation.source_calendar_event_id,
			);
		...
		if (groupSize > 1) {
			...
			reasons.push(
				groupSize > 2
					? "similar items also exist"
					: "grouped with a similar item",
			);
		}
		...
		if (
			...
			if (
				calibration.handled_elsewhere_count / calibration.closed_count >=
				0.5
			) {
				score -= 15;
	...
	private refreshPlanningRecommendationReadModel() {
		...
		const pendingGroups = this.groupPlanningRecommendations(
			activeRecommendations,
		);
		const groupCounts = new Map(
			pendingGroups.map((group) => [group.group_key, group.pending_count]),
		);
		for (const recommendation of activeRecommendations) {
			const groupKind = this.asPlanningRecommendationGroupKind(
				recommendation.group_key ?? "urgent_unscheduled_tasks",
			);
			const groupSize =
				groupCounts.get(groupKind) ??
				(recommendation.status === "pending" ? 1 : 0);
			const familyKey = this.getPlanningCalibrationFamilyKey(recommendation);
			const rank = this.calculateRecommendationRank(
				recommendation,
				groupSize,
				calibrationByFamily.get(familyKey),
			);
			this.db.updatePlanningRecommendation(recommendation.recommendation_id, {
	...
	private compareNextActionableRecommendations(
		left: PlanningRecommendation,
		right: PlanningRecommendation,
	): number {
		const statusRank = (recommendation: PlanningRecommendation) =>
			recommendation.status === "pending"
				? 0
				: recommendation.status === "snoozed"
					? 1
	...
	private buildTaskBlockCandidate(
		task: TaskItem,
		options?: {
		...
		if (options?.proposed_start_at && options?.proposed_end_at) {
			slot = {
				start_at: options.proposed_start_at,
				end_at: options.proposed_end_at,
			};
		...
		return {
			kind: "schedule_task_block",
			priority:
				options?.source === "assistant_created" ? task.priority : task.priority,
			source_task_id: task.task_id,
			proposed_calendar_id: options?.proposed_calendar_id,
			proposed_start_at: slot?.start_at,
			proposed_end_at: slot?.end_at,
			proposed_title:
				String(options?.proposed_title ?? task.title).trim() || task.title,
			...
			reason_summary:
				...
				`Reserve time for high-priority task: ${task.title}`,
			dedupe_key: this.makeRecommendationDedupeKey("schedule_task_block", {
				task_id: task.task_id,
			}),
			source_fingerprint: this.makeTaskRecommendationFingerprint(task),
			...
			slot_state_reason: slot
				? undefined
				: "no_unique_window_after_group_reservation",
			slot_reason: options?.slot_reason ?? "earliest_free_before_due",
			trigger_signals:
				options?.source === "assistant_created"
	...
	private buildThreadFollowupCandidate(
		...
		if (
			...
			this.hasActiveTaskForThread(summary.thread.thread_id)
		) {
			return null;
		}
		...
		const subject =
			summary.latest_message?.subject?.trim() || "Inbox follow-up";
		return {
			...
			source_last_seen_at: this.mailTimestampToIso(
				summary.latest_message?.internal_date ?? "",
				summary.thread.last_synced_at,
			),
			slot_state: slot ? "ready" : "needs_manual_scheduling",
			slot_state_reason: slot
	...
	private buildCandidateFromRecommendation(
		recommendation: PlanningRecommendation,
	): PlanningRecommendationCandidate | null {
		const excludedSlots =
			recommendation.proposed_start_at && recommendation.proposed_end_at
				...
				: undefined;
		if (recommendation.kind === "schedule_task_block") {
			const task = recommendation.source_task_id
				? this.db.getTask(recommendation.source_task_id)
				: null;
			if (!task) {
				return null;
			}
		...
		if (recommendation.kind === "schedule_thread_followup") {
			const thread = recommendation.source_thread_id
				? this.db.getMailThread(recommendation.source_thread_id)
				: null;
			if (!thread) {
				return null;
			}
			return this.buildThreadFollowupCandidate(
				this.buildInboxThreadSummary(thread),
				{ excluded_slots: excludedSlots },
			);
		}
		...
		return this.buildEventPrepCandidate(event, {
			excluded_slots: excludedSlots,
		});
	}
	...
	private upsertPlanningRecommendation(
		identity: ClientIdentity,
		input: PlanningRecommendationCandidate & {
			source: PlanningRecommendation["source"];
		},
	): PlanningRecommendation {
		const existing = this.db.getLatestPlanningRecommendationByDedupeKey(
			input.dedupe_key,
		);
		const now = new Date().toISOString();
		if (existing) {
			const sameFingerprint =
				existing.source_fingerprint === input.source_fingerprint;
			const snoozeExpired =
				!existing.snoozed_until ||
				Date.parse(existing.snoozed_until) <= Date.now();
			...
			if (["pending", "snoozed"].includes(existing.status)) {
				return this.db.updatePlanningRecommendation(
					existing.recommendation_id,
					{
						kind: input.kind,
						status:
							existing.status === "snoozed" && sameFingerprint && !snoozeExpired
								? "snoozed"
								: "pending",
						priority: input.priority,
			...
			if (
				!sameFingerprint &&
				["applied", "expired", "superseded", "rejected"].includes(
					existing.status,
				)
			) {
	...
	private refreshPlanningRecommendationsInternal(identity: ClientIdentity): {
		...
		for (const recommendation of this.db.listPlanningRecommendations({
			include_resolved: false,
		})) {
			...
			if (!this.isRecommendationSourceRelevant(recommendation)) {
				this.markPlanningRecommendationSourceResolved(
					recommendation,
					recommendation.outcome_summary ??
						"The source no longer needs planning action.",
					"system",
					"system",
					identity.requested_by,
				);
		...
		for (const task of tasks) {
			...
			if (candidate) {
				...
				if (candidate.proposed_start_at && candidate.proposed_end_at) {
					reservedSlots.push({
						start_at: candidate.proposed_start_at,
						end_at: candidate.proposed_end_at,
					});
				}
			}
		}
	...
	private collectThreadFollowupRecommendationCandidates(): PlanningRecommendationCandidate[] {
		...
		const summaries = this.listInboxThreadSummaries(
			...
			(left, right) => {
				...
				const priorityDelta =
					priorityRank(left.derived_kind) - priorityRank(right.derived_kind);
				if (priorityDelta !== 0) {
					return priorityDelta;
				}
				const leftSeen = Date.parse(
					this.mailTimestampToIso(
						left.latest_message?.internal_date ?? "",
						left.thread.last_synced_at,
					),
				);
				const rightSeen = Date.parse(
					this.mailTimestampToIso(
						right.latest_message?.internal_date ?? "",
						right.thread.last_synced_at,
					),
				);
	...
	private collectEventPrepRecommendationCandidates(): PlanningRecommendationCandidate[] {
		...
		for (const event of this.listUpcomingCalendarEvents(1, MAX_INBOX_LIMIT)) {
			...
			if (
				...
				now + this.config.meetingPrepWarningMinutes * 60 * 1000
			) {
				break;
			}
	...
	private resolvePlanningSnoozeUntil(
		until?: string,
		preset?: string,
		now = new Date(),
	): string {
		...
		if (!normalizedPreset) {
			throw new Error(
				"Either a snooze preset or snooze-until timestamp is required.",
			);
		}
		...
		if (["end-of-day", "end_of_day", "eod"].includes(normalizedPreset)) {
			...
			resolvedAt =
				todayEnd > now
					...
					: this.nextBusinessWorkdayBoundary(now, "end");
		}
		if (
			!resolvedAt &&
			["tomorrow-morning", "tomorrow_morning"].includes(normalizedPreset)
		) {
			const tomorrow = new Date(now);
			tomorrow.setDate(tomorrow.getDate() + 1);
			resolvedAt = this.getWorkdayBounds(
				this.formatLocalDay(tomorrow),
			).start.toISOString();
		}
		if (
			!resolvedAt &&
			["next-business-day", "next_business_day"].includes(normalizedPreset)
		) {
	...
	private nextBusinessWorkdayBoundary(
		from: Date,
		boundary: "start" | "end",
	): string {
		...
		while (true) {
			...
			const day = cursor.getDay();
			if (day !== 0 && day !== 6) {
				return this.getWorkdayBounds(this.formatLocalDay(cursor))[
					boundary
				].toISOString();
			}
		}
			...
			for (const segment of segments) {
				...
				if (excludedStart > segment.startMs) {
					nextSegments.push({
						...
						endMs: Math.min(excludedStart, segment.endMs),
					});
				}
				if (excludedEnd < segment.endMs) {
					nextSegments.push({
						startMs: Math.max(excludedEnd, segment.startMs),
						endMs: segment.endMs,
					});
				}
			}
	...
	private findFreeWindow(options: {
		...
		return options.preferLatest
			? (windows.sort(
					(left, right) => Date.parse(right.end_at) - Date.parse(left.end_at),
				)[0] ?? null)
			: (windows.sort(
					(left, right) =>
	...
	private hasActiveTaskForThread(threadId: string): boolean {
		return this.db
			.listTasks({ activeOnly: true })
			.some((task) => task.source_thread_id === threadId);
	}

	private hasActiveTaskForEvent(eventId: string): boolean {
		return this.db
			.listTasks({ activeOnly: true })
			.some((task) => task.source_calendar_event_id === eventId);
	}

	private isTimeRangeUsable(
		startAt: string,
		endAt: string,
	...
	private isRecommendationSourceRelevant(
		recommendation: PlanningRecommendation,
	): boolean {
		if (recommendation.kind === "schedule_task_block") {
			const task = recommendation.source_task_id
	...
	private async applyTaskBlockRecommendation(
		...
		if (
			...
			this.db.updatePlanningRecommendation(recommendation.recommendation_id, {
				...
				last_error_message:
					"This recommendation needs manual scheduling before it can be applied.",
			});
			throw new Error(
				"This recommendation needs manual scheduling before it can be applied.",
			);
		...
		const result = await this.scheduleTaskOnCalendar(
			identity,
			task.task_id,
			{
	...
	private async applyThreadFollowupRecommendation(
		...
		if (!this.isRecommendationSlotUsable(recommendation)) {
			this.db.updatePlanningRecommendation(recommendation.recommendation_id, {
				last_error_code: "slot_unavailable",
				last_error_message:
					"The proposed follow-up block is no longer available.",
			});
			throw new Error("The proposed follow-up block is no longer available.");
		}
		const task = this.createTaskFromPlanningRecommendation(
			identity,
			recommendation,
			{
	...
	private requiresStepUp(identity: ClientIdentity): boolean {
		return (
			identity.auth_role === "assistant" || identity.origin === "assistant-mcp"
		);
	}
	...
	private assertStepUpIfRequired(
		...
		if (!confirmationToken) {
			throw new Error(
				`A confirmation token is required to ${action} from MCP.`,
			);
		}
		...
		const consumed = this.db.consumeApprovalConfirmation(
			context.approval.approval_id,
			expected,
			new Date().toISOString(),
		);
		if (!consumed) {
			throw new Error(
				"The confirmation token does not match the current approval state.",
			);
		}
	}
	...
	private assertApprovalActionAllowed(
		context: ApprovalContext,
		action: ApprovalAction,
	) {
		if (
			action === "approve" &&
			!["pending", "send_failed"].includes(context.approval.state)
		) {
			throw new Error(
				`Approval request ${context.approval.approval_id} cannot be approved from state ${context.approval.state}.`,
			);
		}
		if (action === "send" && context.approval.state !== "approved") {
			throw new Error(
				`Approval request ${context.approval.approval_id} cannot be sent from state ${context.approval.state}.`,
			);
		}
	}
	...
	private assertApprovalFresh(context: ApprovalContext) {
		if (Date.parse(context.approval.expires_at) < Date.now()) {
			this.expireApprovalContext(
				context,
				"approval_expired",
				"Approval expired before send.",
			);
			throw new Error(
				`Approval request ${context.approval.approval_id} has expired.`,
			);
		}
	}
	...
	private assertDraftMatchesApproval(context: ApprovalContext) {
		...
		if (currentDigest !== context.approval.draft_digest) {
			...
			throw new Error(
				"The draft content changed after approval was requested. Request approval again.",
			);
		}
	}
	...
	private assertStoredMailboxMatches(connectedMailbox: string) {
		if (!this.config.gmailAccountEmail) {
			throw new Error(
				"A configured Gmail mailbox is required before live sends can be attempted.",
			);
		}
	...
	private normalizePlanningDecisionReasonCode(
		reasonCode?: string,
	): string | null {
		if (!reasonCode?.trim()) {
			return null;
	...
	private normalizePlanningGroupRejectReasonCode(
		reasonCode: string,
	): "duplicate" | "handled_elsewhere" {
		const normalized = String(reasonCode ?? "")
			.trim()
			.toLowerCase();
		if (normalized !== "duplicate" && normalized !== "handled_elsewhere") {
			throw new Error(
				"Grouped planning rejection only supports duplicate or handled_elsewhere.",
			);
		}
		return normalized;
	}
	...
	private applyPlanningRecommendationRejection(
		...
		const outcomeState =
			reasonCode === "handled_elsewhere" ? "handled_elsewhere" : "dismissed";
	...
	private updateRecommendationOutcomeFromTask(
		...
		const recommendation = this.db.getPlanningRecommendation(
			task.source_planning_recommendation_id,
		);
	...
	private expireElapsedSendWindows() {
		const expiredCount = this.db.expireActiveSendWindows(
			new Date().toISOString(),
		);
		if (expiredCount > 0) {
			this.db.recordAuditEvent({
	...
	private recoverStaleSendingApprovals() {
		const sendingApprovals = this.db.listApprovalRequests({
			state: "sending",
			limit: 1000,
		});
		for (const approval of sendingApprovals) {
			...
			if (
				...
				Date.now() - lastAttempt >= SENDING_RECOVERY_MINUTES * 60 * 1000
			) {
	...
	private recoverApprovalAsInterrupted(context: ApprovalContext) {
		const message =
			"The prior send attempt may have been interrupted. Inspect Sent mail before retrying.";
		this.db.updateApprovalRequest(context.approval.approval_id, {
			state: "send_failed",
	...
	private isTrackedMessage(message: GmailMessageMetadata): boolean {
		return (
			message.label_ids.includes("INBOX") || message.label_ids.includes("SENT")
		);
	}
	...
	private isHistoryInvalidError(error: unknown): boolean {
		...
		return (
			code === 404 || /history/i.test(text) || /startHistoryId/i.test(text)
		);
	}
	...
	private async refreshCalendarWindow(
		...
		do {
			const page: GoogleCalendarListPage =
				await this.dependencies.listGoogleCalendarSources(
					tokensJson,
					clientConfig,
					pageToken,
				);
		...
		const selectedSources = allSources.filter((source) =>
			this.isIncludedCalendar(source),
		);
		const now = new Date();
		const timeMin = new Date(
			now.getTime() - this.config.calendarSyncPastDays * 24 * 60 * 60 * 1000,
		).toISOString();
		const timeMax = new Date(
			now.getTime() + this.config.calendarSyncFutureDays * 24 * 60 * 60 * 1000,
		).toISOString();
		...
		for (const source of selectedSources) {
			...
			do {
				const page: GoogleCalendarEventsPage =
					await this.dependencies.listGoogleCalendarEvents(
						tokensJson,
						clientConfig,
						source.calendar_id,
						eventPageToken
				...
				for (const event of page.events) {
					...
					const localEventId = this.makeCalendarEventId(
						source.calendar_id,
						event.event_id,
					);
		...
		this.db.replaceCalendarSources(
			account,
			this.config.calendarProvider,
			selectedSources,
			syncedAt,
		);
		this.db.replaceCalendarEvents(
			account,
			this.config.calendarProvider,
			events,
			syncedAt,
	...
	private async loadCalendarWriteContext(
		calendarId?: string,
	): Promise<{ stored: StoredGmailAuth; target: CalendarSource }> {
		if (!this.config.calendarEnabled) {
			throw new Error(
				"Calendar writes are unavailable because the calendar subsystem is disabled.",
			);
		}
		const stored = await this.dependencies.loadStoredGmailTokens(
			this.config,
	...
	private resolveOwnedCalendarTarget(
		account: string,
		...
		if (owned.length === 0) {
			throw new Error(
				"No owned writable calendars are available for calendar writes.",
			);
		}
		if (requestedCalendarId) {
			const explicit = owned.find(
				(source) => source.calendar_id === requestedCalendarId,
			);
			if (!explicit) {
				throw new Error(
					`Calendar ${requestedCalendarId} is not writable through personal-ops.`,
				);
			}
			return explicit;
		}
	...
	private toLocalCalendarEvent(
		...
		return {
			event_id:
				overrides.event_id ??
				this.makeCalendarEventId(calendarId, event.event_id),
			provider_event_id: event.event_id,
			...
			created_by_personal_ops:
				overrides.created_by_personal_ops ?? event.created_by_personal_ops,
	...
	private assertTimedCalendarRange(
		startAtRaw?: string,
		endAtRaw?: string,
		...
		if (
			...
			throw new Error(
				"Calendar event timestamps must be valid UTC timestamps.",
			);
		}
	...
	private normalizeCalendarUpdateInput(
		...
		if (input.start_at !== undefined || input.end_at !== undefined) {
			const { startAt, endAt } = this.assertTimedCalendarRange(
				input.start_at ?? current.start_at,
				input.end_at ?? current.end_at,
			);
	...
	private assertCalendarEventMutable(event: CalendarEvent): void {
		if (event.is_all_day) {
			throw new Error(
				"All-day calendar events are not editable through personal-ops in Phase 7.",
			);
		}
		if (event.recurring_event_id) {
			throw new Error(
				"Recurring calendar events are not editable through personal-ops in Phase 7.",
			);
		}
		if (event.event_type && event.event_type !== "default") {
			throw new Error(
				`Calendar event type ${event.event_type} is not editable through personal-ops in Phase 7.`,
			);
		}
	}

	private assertCalendarEventIsCurrent(
		local: CalendarEvent,
		provider: GoogleCalendarEventMetadata,
	): void {
		if (local.etag && provider.etag && local.etag !== provider.etag) {
			throw new Error(
				`Calendar event ${local.event_id} has changed in Google Calendar. Run personal-ops calendar sync now first.`,
			);
		}
		if (local.updated_at !== provider.updated_at) {
			throw new Error(
				`Calendar event ${local.event_id} is stale locally. Run personal-ops calendar sync now first.`,
			);
		}
	}

	private computeCalendarConflicts(
		...
		for (let index = 0; index < busyEvents.length; index += 1) {
			...
			for (
				...
				compareIndex < busyEvents.length;
				compareIndex += 1
			) {
				const right = busyEvents[compareIndex]!;
				const overlapStart = Math.max(
					Date.parse(left.start_at),
					Date.parse(right.start_at),
				);
				const overlapEnd = Math.min(
					Date.parse(left.end_at),
					Date.parse(right.end_at),
				);
	...
	private hasCalendarConflictForEvent(event: CalendarEvent): boolean {
		...
		return this.computeCalendarConflicts(
			...
			(conflict) =>
				conflict.left_event.event_id === event.event_id ||
				conflict.right_event.event_id === event.event_id,
		);
	}

	private computeFreeTimeWindows(
		day: string,
	...
	private getWorkdayBounds(day: string): { start: Date; end: Date } {
		...
		const month = Number.isFinite(Number(parts[1])) ? Number(parts[1]) : 1;
		const date = Number.isFinite(Number(parts[2])) ? Number(parts[2]) : 1;
		const [startHour, startMinute] = this.parseLocalHourMinute(
			this.config.workdayStartLocal,
		);
		const [endHour, endMinute] = this.parseLocalHourMinute(
			this.config.workdayEndLocal,
		);
		return {
	...
	private async seedMailboxMetadata(
		...
		for (const labelId of ["INBOX", "SENT"]) {
			let pageToken: string | undefined;
			do {
				const page = await this.dependencies.listGmailMessageRefsByLabel(
					tokensJson,
					clientConfig,
					labelId,
					pageToken,
		...
		for (const messageId of messageIds) {
			const message = await this.dependencies.getGmailMessageMetadata(
				tokensJson,
				clientConfig,
				messageId,
			);
			...
			if (
				message.history_id &&
				(!historyId || Number(message.history_id) > Number(historyId))
			) {
		...
		const resolvedHistoryId = profile.profile.historyId
			? String(profile.profile.historyId)
			: historyId;
	...
	private async incrementalMailboxSync(
		...
		try {
			do {
				const page: GmailHistoryPage = await this.dependencies.listGmailHistory(
					tokensJson,
					clientConfig,
					latestHistoryId,
					pageToken,
			...
			if (this.isHistoryInvalidError(error)) {
				throw new Error(
					"History id is stale. Run a full reseed with personal-ops inbox sync now.",
				);
			}
			throw error;
		}
	...
	private inspectLaunchAgent() {
		return this.dependencies.inspectLaunchAgent(
			undefined,
			getLaunchAgentLabel(),
		);
	}
	...
	private isOAuthClientConfigured(): boolean {
		return (
			validateOAuthClientFile(this.config.oauthClientFile).status ===
			"configured"
		);
	}
	...
	private fileCheck(
		...
		try {
			...
			return this.passCheck(
				id,
				title,
				`${path.basename(filePath)} is present and readable.`,
				"runtime",
			);
		} catch (error) {
			return this.failCheck(
				...
				error instanceof Error
					? error.message
					: `${filePath} could not be read.`,
				"runtime",
			);
		}
	}

	private directoryWritableCheck(
		id: string,
		title: string,
		directory: string,
	): DoctorCheck {
	...
	private snapshotFreshnessCheck(
