...
const GOOGLE_CALENDAR_OWNED_SCOPE =
	"https://www.googleapis.com/auth/calendar.events.owned";
const GOOGLE_CALENDAR_BROAD_SCOPE =
	"https://www.googleapis.com/auth/calendar.events";

function createAuthorizedOAuthClient(
	tokensJson: string,
...
export async function verifyGoogleCalendarWriteAccess(
	tokensJson: string,
	clientConfig: GmailClientConfig,
) {
	const oauthClient = createAuthorizedOAuthClient(tokensJson, clientConfig);
	...
	if (!rawToken) {
		throw new Error(
			"Google access token could not be refreshed for calendar write verification.",
		);
	}
	...
	const scopes = String(info.data.scope ?? "")
		...
		.filter(Boolean);
	if (
		!scopes.includes(GOOGLE_CALENDAR_OWNED_SCOPE) &&
		!scopes.includes(GOOGLE_CALENDAR_BROAD_SCOPE)
	) {
		throw new Error(
			"Google token is missing the calendar.events.owned write scope.",
		);
	}
}
...
export async function getGoogleCalendarEvent(
	...
	if (!response.data.id || !response.data.start || !response.data.end) {
		throw new Error(
			`Google Calendar event ${providerEventId} could not be loaded for mutation.`,
		);
	}
...
export async function createGoogleCalendarEvent(
		...
		extendedProperties: {
			private: {
				...
				...(input.source_task_id
					? { po_source_task_id: input.source_task_id }
					: {}),
				...(input.created_by_client
					? { po_created_by: input.created_by_client }
					: {}),
			},
		},
	};
	...
	if (!response.data.id || !response.data.start || !response.data.end) {
		throw new Error(
			"Google Calendar did not return a complete event after creation.",
		);
	}
	return parseGoogleCalendarEvent(calendarId, response.data);
}
...
export async function patchGoogleCalendarEvent(
	...
	if (input.start_at !== undefined)
		requestBody.start = { dateTime: input.start_at };
	if (input.end_at !== undefined) requestBody.end = { dateTime: input.end_at };
	if (
		input.source_task_id !== undefined ||
		input.created_by_client !== undefined
	) {
	...
	if (!response.data.id || !response.data.start || !response.data.end) {
		throw new Error(
			`Google Calendar did not return a complete event after updating ${providerEventId}.`,
		);
	}
	return parseGoogleCalendarEvent(calendarId, response.data);
}
