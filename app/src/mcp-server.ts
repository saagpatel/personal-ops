...
function assertAllowedToolArgs(args: Record<string, unknown>, allowedKeys: string[], toolName: string) {
  const allowed = new Set(allowedKeys);
  const unsupported = [...new Set(Object.keys(args).filter((key) => !allowed.has(key)))];
...
const tools = [
  {
    ...
    description: "Create a Gmail draft through the local personal-ops control plane.",
    inputSchema: {
      type: "object",
      required: ["to", "subject"],
      properties: {
  ...
  },
  {
    ...
    description: "Create a send approval request for a draft when directly instructed by the user.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description: "Approve a request only when the user directly instructs the action. Requires a confirmation token.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description: "Reject a request only when the user directly instructs the action.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description: "Send an approved draft only when the user directly instructs the action. Requires a confirmation token.",
    inputSchema: {
      type: "object",
      required: ["approval_id", "note", "confirmation_token"],
      properties: {
  ...
  {
    name: "inbox_status",
    description: "Show the current mailbox metadata sync status and inbox counts.",
    inputSchema: {
      type: "object",
      properties: {},
      additionalProperties: false,
  ...
  },
  {
    ...
    description: "List sent follow-up threads that have not received a newer inbound reply.",
    inputSchema: {
      type: "object",
      properties: {
        limit: { type: "number" },
  ...
  },
  {
    ...
    description: "List inbox threads whose latest message is inbound and likely waiting on the operator.",
    inputSchema: {
      type: "object",
      properties: {
        limit: { type: "number" },
  ...
  },
  {
    ...
    description: "List recently updated mailbox threads across the local INBOX and SENT metadata index.",
    inputSchema: {
      type: "object",
      properties: {
        limit: { type: "number" },
  ...
  },
  {
    ...
    description: "Show the current shared calendar sync status and scheduling summary.",
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
  },
  {
    name: "calendar_calendars_list",
    description: "List calendars currently selected in the local calendar index.",
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
  },
  {
  ...
  },
  {
    ...
    description: "Create a task suggestion when the user directly instructs you to remember or track something.",
    inputSchema: {
      type: "object",
      required: ["title"],
      properties: {
  ...
  },
  {
    ...
    description: "Get the next actionable planning recommendation overall or within a group.",
    inputSchema: {
      type: "object",
      properties: {
        group: { type: "string" },
  ...
  },
  {
    ...
    description: "Get operator-facing planning tuning summaries for reviewed and proposed hygiene families.",
    inputSchema: {
      type: "object",
      properties: {},
      additionalProperties: false,
  ...
  },
  {
    ...
    description: "Get planning recommendation backlog analytics overall or for one group.",
    inputSchema: {
      type: "object",
      properties: {
        group: { type: "string" },
  ...
  },
  {
    ...
    description: "Get planning recommendation closure analytics for a recent lookback window.",
    inputSchema: {
      type: "object",
      properties: {
        days: { type: "number" },
  ...
  },
  {
    ...
    description: "Get planning recommendation hygiene analytics and advisory suppression candidates.",
    inputSchema: {
      type: "object",
      properties: {
        group: { type: "string" },
  ...
  },
  {
    ...
    description: "Create a scheduling recommendation for an existing task when directly instructed by the user.",
    inputSchema: {
      type: "object",
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "mail_draft_update") {
    ...
    const response = await requestJson("PATCH", `/v1/mail/drafts/${artifactId}`, body);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  ...
  if (name === "approval_queue_list") {
    ...
    const response = await requestJson("GET", `/v1/approval-queue?${search.toString()}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  ...
  if (name === "approval_queue_get") {
    const response = await requestJson("GET", `/v1/approval-queue/${String(args.approval_id)}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "approval_request_create") {
    const response = await requestJson("POST", `/v1/mail/drafts/${String(args.artifact_id)}/request-approval`, {
      note: args.note,
    });
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "approval_request_approve") {
    const response = await requestJson("POST", `/v1/approval-queue/${String(args.approval_id)}/approve`, {
      note: args.note,
  ...
  if (name === "approval_request_reject") {
    const response = await requestJson("POST", `/v1/approval-queue/${String(args.approval_id)}/reject`, {
      note: args.note,
    });
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "approval_request_send") {
    const response = await requestJson("POST", `/v1/approval-queue/${String(args.approval_id)}/send`, {
      note: args.note,
      confirmation_token: args.confirmation_token,
    });
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  ...
  if (name === "audit_events_recent") {
    ...
    const response = await requestJson("GET", `/v1/audit/events?${search.toString()}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  ...
  if (name === "personal_ops_doctor") {
    ...
    const response = await requestJson("GET", deep ? "/v1/doctor?deep=true" : "/v1/doctor");
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  ...
  if (name === "review_queue_get") {
    const response = await requestJson("GET", `/v1/review-queue/${String(args.review_id)}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  ...
  if (name === "inbox_unread_list") {
    ...
    const response = await requestJson("GET", `/v1/inbox/unread?limit=${limit}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "inbox_followup_list") {
    const limit = Number(args.limit ?? 50);
    const response = await requestJson("GET", `/v1/inbox/followups?limit=${limit}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "inbox_thread_get") {
    const response = await requestJson("GET", `/v1/inbox/threads/${String(args.thread_id)}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "inbox_needs_reply_list") {
    const limit = Number(args.limit ?? 50);
    const response = await requestJson("GET", `/v1/inbox/needs-reply?limit=${limit}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "inbox_recent_list") {
    const limit = Number(args.limit ?? 50);
    const response = await requestJson("GET", `/v1/inbox/recent?limit=${limit}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  ...
  if (name === "calendar_upcoming_list") {
    ...
    const response = await requestJson("GET", `/v1/calendar/upcoming?days=${days}&limit=${limit}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "calendar_conflict_list") {
    const days = Number(args.days ?? 7);
    const response = await requestJson("GET", `/v1/calendar/conflicts?days=${days}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "calendar_free_time_get") {
    const response = await requestJson("GET", `/v1/calendar/free-time?day=${encodeURIComponent(String(args.day))}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "calendar_day_get") {
    const response = await requestJson("GET", `/v1/calendar/day?day=${encodeURIComponent(String(args.day))}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "calendar_event_get") {
    const response = await requestJson("GET", `/v1/calendar/events/${encodeURIComponent(String(args.event_id))}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "task_list") {
  ...
  if (name === "task_get") {
    const response = await requestJson("GET", `/v1/tasks/${String(args.task_id)}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  ...
  if (name === "task_suggestion_get") {
    const response = await requestJson("GET", `/v1/task-suggestions/${String(args.suggestion_id)}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  ...
  if (name === "planning_recommendation_list") {
    ...
    const response = await requestJson("GET", `/v1/planning-recommendations${suffix}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "planning_recommendation_get") {
    const response = await requestJson("GET", `/v1/planning-recommendations/${String(args.recommendation_id)}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "planning_recommendation_group_list") {
    ...
    if (args.all) search.set("all", "true");
    const suffix = search.size ? `?${search.toString()}` : "";
    const response = await requestJson("GET", `/v1/planning-recommendation-groups${suffix}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "planning_recommendation_group_get") {
    const response = await requestJson("GET", `/v1/planning-recommendation-groups/${encodeURIComponent(String(args.group_key))}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "planning_recommendation_next") {
    ...
    const suffix = search.size ? `?${search.toString()}` : "";
    const response = await requestJson("GET", `/v1/planning-recommendations/next${suffix}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "planning_recommendation_summary") {
    const response = await requestJson("GET", "/v1/planning-recommendations/summary");
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "planning_recommendation_backlog") {
    ...
    const suffix = search.size ? `?${search.toString()}` : "";
    const response = await requestJson("GET", `/v1/planning-recommendations/backlog${suffix}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "planning_recommendation_closure") {
    ...
    if (args.kind) search.set("kind", String(args.kind));
    if (args.source) search.set("source", String(args.source));
    if (args.close_reason) search.set("close_reason", String(args.close_reason));
    const suffix = search.size ? `?${search.toString()}` : "";
    const response = await requestJson("GET", `/v1/planning-recommendations/closure${suffix}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "planning_recommendation_hygiene") {
    ...
    const suffix = search.size ? `?${search.toString()}` : "";
    const response = await requestJson("GET", `/v1/planning-recommendations/hygiene${suffix}`);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "planning_recommendation_tuning") {
    const response = await requestJson("GET", "/v1/planning-recommendations/tuning");
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
  if (name === "planning_recommendation_create") {
    const response = await requestJson("POST", "/v1/planning-recommendations", args);
    return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
  }
