...
import type { PersonalOpsService } from "./service.js";
...
async function readJsonBody(request: http.IncomingMessage): Promise<any> {
	...
	for await (const chunk of request) {
		...
		if (totalBytes > MAX_JSON_BODY_BYTES) {
			throw new HttpError(
				413,
				`JSON request body exceeds the ${MAX_JSON_BODY_BYTES}-byte limit.`,
			);
		}
		chunks.push(buffer);
	}
...
function extractIdentity(
	...
	const browserSessionId = (
		request as http.IncomingMessage & { browserSessionId?: string }
	).browserSessionId;
	if (browserSessionId) {
		return {
	...
	return {
		client_id: String(
			request.headers["x-personal-ops-client"] ?? "unknown-client",
		),
		session_id: request.headers["x-personal-ops-session"]
			? String(request.headers["x-personal-ops-session"])
			: undefined,
		origin:
			authRole === "assistant"
				? "assistant-mcp"
...
function assertAuthorized(
	...
	if (!session) {
		throw new HttpError(
			401,
			"Console session expired. Run `personal-ops console` to reopen the operator console.",
		);
	}
...
function draftInputFromBody(body: any): DraftInput {
	return {
		to: Array.isArray(body.to)
			? body.to.map((item: unknown) => String(item))
			: [],
		cc: Array.isArray(body.cc)
			? body.cc.map((item: unknown) => String(item))
			: [],
		bcc: Array.isArray(body.bcc)
			? body.bcc.map((item: unknown) => String(item))
			: [],
...
function planningRecommendationInputFromBody(body: any) {
	return {
		kind: String(
			body.kind ?? "schedule_task_block",
		) as PlanningRecommendationKind,
...
function planningPolicyPruneInputFromBody(body: any) {
	const eventType =
		body.event_type === undefined ? "all" : String(body.event_type);
	...
	return {
		older_than_days: Number(
			body.older_than_days ?? body.olderThanDays ?? Number.NaN,
		),
		...
		dry_run:
			...
			(typeof rawDryRun === "string" &&
				["1", "true", "yes", "on"].includes(rawDryRun.toLowerCase())),
	};
}
...
function parseAuditCategoryQuery(
	raw: string | null,
...
function assertAllowedQueryParams(url: URL, allowedParams: string[]) {
	...
	const unsupported = [
		...new Set(
			Array.from(url.searchParams.keys()).filter((key) => !allowed.has(key)),
		),
	];
	...
	throw new Error(
		`unsupported query ${label}: ${unsupported.join(", ")}. Only ${allowedParams.join(" and ")} are supported.`,
	);
}
...
function readHubFeed(limit: number): HubEventRecord[] {
	try {
...
export function createHttpServer(
	...
	return http.createServer(async (request, response) => {
		try {
			const url = new URL(
				request.url ?? "/",
				`http://${config.serviceHost}:${config.servicePort}`,
			);
			let auth: RequestAuth | undefined;
			if (!isLocalRequest(request.socket.remoteAddress)) {
				sendJson(response, 403, {
					error: "Only localhost requests are allowed.",
				});
				return;
			}
			...
			if (
				request.method === "GET" &&
				url.pathname.startsWith("/console/session/")
			) {
				const grant = decodeURIComponent(
					url.pathname.slice("/console/session/".length),
				);
				const session =
					grant && !grant.includes("/")
						? sessionStore.consumeGrant(grant)
						: null;
				if (!session) {
			...
			if (
				request.method === "POST" &&
				(url.pathname === "/v1/web/session-grants" ||
					url.pathname === "/v1/console/session")
			) {
				if (!auth || auth.source !== "bearer" || auth.role !== "operator") {
					throw new HttpError(
						403,
						"Console session grants require the operator bearer token.",
					);
				}
			...
			if (request.method === "GET" && url.pathname === "/v1/status") {
				const skipDerived =
					url.searchParams.get("skip_derived") === "1" ||
					url.searchParams.get("skipDerived") === "1";
				sendJson(response, 200, {
					status: await service.getStatusReport({
						httpReachable: true,
						skipDerived,
					}),
				});
				...
				const warmTrigger =
					auth?.source === "browser_session"
						? config.autopilotWarmOnConsoleOpen
							? "console_open"
							: null
						: request.headers["x-personal-ops-origin"] === "desktop-shell" &&
								config.autopilotWarmOnDesktopOpen
							? "desktop_open"
							: null;
				sendJson(response, 200, {
					autopilot: await service.getAutopilotStatusReport({
						httpReachable: true,
						triggerWarm: warmTrigger as any,
					}),
				});
				return;
			}
			...
			if (request.method === "POST" && url.pathname === "/v1/autopilot/run") {
				if (!auth || auth.source !== "bearer" || auth.role !== "operator") {
					throw new HttpError(
						403,
						"Autopilot runs require the operator bearer token.",
					);
				}
				sendJson(response, 200, {
					autopilot: await service.runAutopilot(
			...
			if (
				request.method === "POST" &&
				url.pathname.startsWith("/v1/autopilot/run/")
			) {
				const profile = decodeURIComponent(
					url.pathname.slice("/v1/autopilot/run/".length),
				);
				if (profile && !profile.includes("/")) {
					if (
						...
						throw new HttpError(
							400,
							"Autopilot profile must be day_start, inbox, meetings, planning, or outbound.",
						);
					}
					if (!auth || auth.source !== "bearer" || auth.role !== "operator") {
						throw new HttpError(
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/assistant/actions"
			) {
			...
			if (
				request.method === "POST" &&
				url.pathname.startsWith("/v1/assistant/actions/") &&
				url.pathname.endsWith("/run")
			) {
				const actionId = decodeURIComponent(
					url.pathname
						.replace("/v1/assistant/actions/", "")
						.replace("/run", ""),
				);
			...
			if (
				...
				sendJson(response, 200, {
					workflow: await service.getPrepDayWorkflowReport({
						httpReachable: true,
					}),
				});
				return;
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/workflows/now-next"
			) {
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/workflows/follow-up-block"
			) {
				sendJson(response, 200, {
					workflow: await service.getFollowUpBlockWorkflowReport({
						httpReachable: true,
					}),
				});
				return;
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/workflows/prep-meetings"
			) {
				const scope =
					url.searchParams.get("scope") === "next_24h" ? "next_24h" : "today";
				sendJson(response, 200, {
					workflow: await service.getPrepMeetingsWorkflowReport({
						httpReachable: true,
						scope,
					}),
				});
			...
			if (
				request.method === "GET" &&
				url.pathname.startsWith("/v1/workflows/prep-meetings/")
			) {
				const eventId = decodeURIComponent(
					url.pathname.slice("/v1/workflows/prep-meetings/".length),
				);
			...
			if (
				...
				const eventId = decodeURIComponent(
					url.pathname.slice(
						"/v1/workflows/prep-meetings/".length,
						-"/prepare".length,
					),
				);
				if (eventId && !eventId.includes("/")) {
			...
			if (request.method === "POST" && url.pathname === "/v1/github/sync") {
				sendJson(response, 200, {
					github: await service.syncGithub(
						extractIdentity(request, auth?.role ?? "operator"),
					),
				});
				return;
			...
			if (
				request.method === "GET" &&
				url.pathname.startsWith("/v1/github/pulls/")
			) {
				const prKey = decodeURIComponent(
					url.pathname.slice("/v1/github/pulls/".length),
				);
				if (prKey) {
					sendJson(response, 200, {
			...
			if (
				request.method === "GET" &&
				url.pathname.startsWith("/v1/drive/docs/")
			) {
				const fileId = decodeURIComponent(
					url.pathname.slice("/v1/drive/docs/".length),
				);
				if (fileId) {
					sendJson(response, 200, {
			...
			if (
				request.method === "GET" &&
				url.pathname.startsWith("/v1/drive/sheets/")
			) {
				const fileId = decodeURIComponent(
					url.pathname.slice("/v1/drive/sheets/".length),
				);
				if (fileId) {
					sendJson(response, 200, {
			...
			if (request.method === "POST" && url.pathname === "/v1/calendar/sync") {
				sendJson(response, 200, {
					calendar: await service.syncCalendarMetadata(
						extractIdentity(request, auth?.role ?? "operator"),
					),
				});
				return;
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/calendar/calendars"
			) {
				sendJson(response, 200, {
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/calendar/upcoming"
			) {
				sendJson(response, 200, {
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/calendar/conflicts"
			) {
				sendJson(response, 200, {
					conflicts: service.listCalendarConflicts(
						Number(url.searchParams.get("days") ?? 7),
					),
				});
				return;
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/calendar/free-time"
			) {
			...
			if (
				request.method === "GET" &&
				url.pathname.startsWith("/v1/calendar/events/")
			) {
				const eventId = decodeURIComponent(
					url.pathname.slice("/v1/calendar/events/".length),
				);
				if (eventId && !eventId.includes("/")) {
					sendJson(response, 200, {
			...
			if (
				...
				const eventId = decodeURIComponent(
					url.pathname.slice("/v1/calendar/events/".length, -"/cancel".length),
				);
				if (eventId && !eventId.includes("/")) {
					const body = await readJsonBody(request);
					sendJson(response, 200, {
			...
			if (
				request.method === "POST" &&
				url.pathname.startsWith("/v1/calendar/tasks/") &&
				url.pathname.endsWith("/schedule")
			) {
				const taskId = decodeURIComponent(
					url.pathname.slice("/v1/calendar/tasks/".length, -"/schedule".length),
				);
			...
			if (
				request.method === "POST" &&
				url.pathname.startsWith("/v1/calendar/tasks/") &&
				url.pathname.endsWith("/unschedule")
			) {
				const taskId = decodeURIComponent(
					url.pathname.slice(
						"/v1/calendar/tasks/".length,
						-"/unschedule".length,
					),
				);
				if (taskId && !taskId.includes("/")) {
			...
			if (request.method === "POST" && url.pathname === "/v1/inbox/sync") {
				sendJson(response, 200, {
					inbox: await service.syncMailboxMetadata(
						extractIdentity(request, auth?.role ?? "operator"),
					),
				});
				return;
			...
			if (request.method === "GET" && url.pathname === "/v1/inbox/autopilot") {
				sendJson(response, 200, {
					inbox_autopilot: await service.getInboxAutopilotReport({
						httpReachable: true,
					}),
				});
				return;
			...
			if (
				request.method === "POST" &&
				url.pathname.startsWith("/v1/inbox/autopilot/groups/") &&
				url.pathname.endsWith("/prepare")
			) {
				const groupId = decodeURIComponent(
					url.pathname.slice(
						"/v1/inbox/autopilot/groups/".length,
						-"/prepare".length,
					),
				);
			...
			if (request.method === "GET" && url.pathname === "/v1/inbox/unread") {
				sendJson(response, 200, {
					threads: service.listUnreadInboxThreads(
						parseLimit(url.searchParams.get("limit")),
					),
				});
				return;
			...
			if (request.method === "GET" && url.pathname === "/v1/inbox/followups") {
				sendJson(response, 200, {
					threads: service.listFollowupThreads(
						parseLimit(url.searchParams.get("limit")),
					),
				});
				return;
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/inbox/needs-reply"
			) {
				sendJson(response, 200, {
					threads: service.listNeedsReplyThreads(
						parseLimit(url.searchParams.get("limit")),
					),
				});
				return;
			...
			if (request.method === "GET" && url.pathname === "/v1/inbox/recent") {
				sendJson(response, 200, {
					threads: service.listRecentThreads(
						parseLimit(url.searchParams.get("limit")),
					),
				});
				return;
			...
			if (
				request.method === "GET" &&
				url.pathname.startsWith("/v1/inbox/threads/")
			) {
				const threadId = decodeURIComponent(
					url.pathname.slice("/v1/inbox/threads/".length),
				);
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/send-window/enable"
			) {
				const body = await readJsonBody(request);
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/send-window/disable"
			) {
				const body = await readJsonBody(request);
			...
			if (request.method === "GET" && url.pathname === "/v1/doctor") {
				const deep = ["1", "true", "yes"].includes(
					(url.searchParams.get("deep") ?? "").toLowerCase(),
				);
				sendJson(response, 200, {
			...
			if (request.method === "GET" && url.pathname === "/v1/hub/feed") {
				if (!auth) throw new HttpError(401, "Authentication required.");
				const rawLimit = url.searchParams.get("limit");
				const limit = rawLimit
					? Math.min(Math.max(1, parseInt(rawLimit, 10) || 50), 500)
					: 50;
				sendJson(response, 200, { events: readHubFeed(limit) });
				return;
			}
			...
			if (
				...
				if (!Number.isFinite(callbackPort) || callbackPort <= 0) {
					sendJson(response, 400, {
						error: "callback_port must be a valid positive number.",
					});
					return;
				}
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/auth/gmail/callback/complete"
			) {
				const body = await readJsonBody(request);
				sendJson(
					response,
					200,
					await service.completeGmailAuth(
						String(body.state ?? ""),
						String(body.code ?? ""),
					),
				);
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/auth/google/callback/complete"
			) {
				const body = await readJsonBody(request);
				sendJson(
					response,
					200,
					await service.completeGoogleAuth(
						String(body.state ?? ""),
						String(body.code ?? ""),
					),
				);
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/auth/github/login"
			) {
				const body = await readJsonBody(request);
				const token = String(body.token ?? "");
				sendJson(response, 200, {
					github_account: await service.loginGithubPat(
						extractIdentity(request, auth?.role ?? "operator"),
						token,
					),
				});
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/auth/github/logout"
			) {
				sendJson(response, 200, {
					github_logout: service.logoutGithub(
						extractIdentity(request, auth?.role ?? "operator"),
					),
				});
				return;
			...
			if (
				request.method === "PATCH" &&
				url.pathname.startsWith("/v1/mail/drafts/")
			) {
				const artifactId = decodeURIComponent(
					url.pathname.slice("/v1/mail/drafts/".length),
				);
				const body = await readJsonBody(request);
				sendJson(response, 200, {
			...
			if (
				...
				const artifactId = decodeURIComponent(
					url.pathname
						.replace("/v1/mail/drafts/", "")
						.replace("/request-approval", ""),
				);
				const body = await readJsonBody(request);
				sendJson(response, 200, {
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/review-queue/pending"
			) {
				sendJson(response, 200, {
					review_items: service.listPendingReviewQueue(),
				});
				return;
			}
			...
			if (
				request.method === "GET" &&
				url.pathname.startsWith("/v1/review-queue/")
			) {
				const reviewId = decodeURIComponent(
					url.pathname.slice("/v1/review-queue/".length),
				);
				if (reviewId && !reviewId.includes("/")) {
					sendJson(response, 200, {
						review: service.getReviewDetail(reviewId),
					});
					return;
				}
			}
			...
			if (
				...
				const reviewId = decodeURIComponent(
					url.pathname.replace("/v1/review-queue/", "").replace("/open", ""),
				);
				sendJson(
					response,
					200,
			...
			if (
				...
				const reviewId = decodeURIComponent(
					url.pathname.replace("/v1/review-queue/", "").replace("/resolve", ""),
				);
				const body = await readJsonBody(request);
				sendJson(response, 200, {
			...
			if (
				request.method === "GET" &&
				url.pathname.startsWith("/v1/review/packages/")
			) {
				const packageId = decodeURIComponent(
					url.pathname.slice("/v1/review/packages/".length),
				);
			...
			if (
				...
				const packageId = decodeURIComponent(
					url.pathname
						.replace("/v1/review/packages/", "")
						.replace("/feedback", ""),
				);
				if (packageId && !packageId.includes("/")) {
					const body = await readJsonBody(request);
					const feedbackInput = {
						reason: String(body.reason ?? "useful") as
							| "useful"
							| "wrong_priority"
							| "bad_timing"
							| "not_useful",
						note: String(body.note ?? ""),
						...(body.package_item_id
							? { package_item_id: String(body.package_item_id) }
							: {}),
					};
					sendJson(response, 200, {
			...
			if (request.method === "GET" && url.pathname === "/v1/review/report") {
				...
				if (identity.auth_role !== "operator") {
					throw new HttpError(
						403,
						"Only the operator can read the review report.",
					);
				}
				const windowDaysRaw = Number(
					url.searchParams.get("window_days") ?? "14",
				);
			...
			if (request.method === "GET" && url.pathname === "/v1/review/weekly") {
				...
				if (identity.auth_role !== "operator") {
					throw new HttpError(
						403,
						"Only the operator can read the weekly review report.",
					);
				}
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/review/calibration"
			) {
				const identity = extractIdentity(request, auth?.role ?? "operator");
				if (identity.auth_role !== "operator") {
					throw new HttpError(
						403,
						"Only the operator can read review calibration.",
					);
				}
				const surfaceParam = url.searchParams.get("surface");
				const surface =
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/review/calibration/targets"
			) {
				const identity = extractIdentity(request, auth?.role ?? "operator");
				if (identity.auth_role !== "operator") {
					throw new HttpError(
						403,
						"Only the operator can read review calibration targets.",
					);
				}
				sendJson(response, 200, {
			...
			if (
				request.method === "PUT" &&
				url.pathname.startsWith("/v1/review/calibration/targets/")
			) {
				const scopeKey = decodeURIComponent(
					url.pathname.slice("/v1/review/calibration/targets/".length),
				);
				if (scopeKey && !scopeKey.includes("/")) {
					if (auth?.role !== "operator" || auth.source !== "bearer") {
						throw new HttpError(
							403,
							"Only bearer-authenticated operator clients can update review calibration targets.",
						);
					}
					const body = await readJsonBody(request);
					sendJson(response, 200, {
								...
								...(body.max_negative_feedback_rate !== undefined
									? {
											max_negative_feedback_rate: Number(
												body.max_negative_feedback_rate,
											),
										}
									: {}),
								...(body.min_notification_action_conversion_rate !== undefined
									? {
											min_notification_action_conversion_rate: Number(
												body.min_notification_action_conversion_rate,
											),
										}
									: {}),
			...
			if (
				...
				if (scopeKey && !scopeKey.includes("/")) {
					if (auth?.role !== "operator" || auth.source !== "bearer") {
						throw new HttpError(
							403,
							"Only bearer-authenticated operator clients can reset review calibration targets.",
						);
					}
					sendJson(response, 200, {
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/review/notifications"
			) {
				sendJson(response, 200, {
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/review/notifications/events"
			) {
				const body = await readJsonBody(request);
				sendJson(response, 200, {
					recorded: true,
								...
								.recordReviewNotificationEvents(
									...
									body.events.map((event: any) => ({
										...
										...(event.package_id
											...
											: {}),
										...(event.package_cycle_id
											? { package_cycle_id: String(event.package_cycle_id) }
											: {}),
										...(event.proposal_id
											? { proposal_id: String(event.proposal_id) }
											: {}),
										...(event.suppression_reason
											? { suppression_reason: String(event.suppression_reason) }
											: {}),
			...
			if (
				request.method === "POST" &&
				url.pathname.startsWith("/v1/review/tuning/") &&
				url.pathname.endsWith("/approve")
			) {
				const proposalId = decodeURIComponent(
					url.pathname
						.replace("/v1/review/tuning/", "")
						.replace("/approve", ""),
				);
				...
				sendJson(response, 200, {
					tasks: service.listTasks({
						state: (url.searchParams.get("state") ?? undefined) as
							...
							| "canceled"
							| undefined,
						include_history: ["1", "true", "yes"].includes(
				...
				sendJson(response, 200, {
					task: service.createTask(
						extractIdentity(request, auth?.role ?? "operator"),
						{
							...
							kind: String(body.kind ?? "human_reminder") as
								| "human_reminder"
								| "assistant_work",
							priority: String(body.priority ?? "normal") as
								...
								| "normal"
								| "high",
							owner: String(body.owner ?? "operator") as
								| "operator"
								| "assistant",
				...
				sendJson(response, 200, {
					result: service.pruneTaskHistory(
						...
						Array.isArray(body.states)
							? body.states.map(
									(value: unknown) => String(value) as "completed" | "canceled",
								)
							: undefined,
					),
				});
					...
					task: service.updateTask(
						...
						taskId,
						{
							...
							kind:
								body.kind === undefined
									? undefined
									: (String(body.kind) as "human_reminder" | "assistant_work"),
							priority:
								body.priority === undefined
									? undefined
									: (String(body.priority) as "low" | "normal" | "high"),
							owner:
								body.owner === undefined
									? undefined
									: (String(body.owner) as "operator" | "assistant"),
							due_at:
			...
			if (
				...
				const taskId = decodeURIComponent(
					url.pathname.replace("/v1/tasks/", "").replace("/start", ""),
				);
				sendJson(response, 200, {
					task: service.startTask(
						extractIdentity(request, auth?.role ?? "operator"),
						taskId,
					),
				});
			...
			if (
				...
				const taskId = decodeURIComponent(
					url.pathname.replace("/v1/tasks/", "").replace("/complete", ""),
				);
				const body = await readJsonBody(request);
				sendJson(response, 200, {
			...
			if (
				...
				const taskId = decodeURIComponent(
					url.pathname.replace("/v1/tasks/", "").replace("/cancel", ""),
				);
				const body = await readJsonBody(request);
				sendJson(response, 200, {
					task: service.cancelTask(
						extractIdentity(request, auth?.role ?? "operator"),
						taskId,
						String(body.note ?? ""),
					),
			...
			if (
				...
				const taskId = decodeURIComponent(
					url.pathname.replace("/v1/tasks/", "").replace("/snooze", ""),
				);
				const body = await readJsonBody(request);
				sendJson(response, 200, {
				...
				sendJson(response, 200, {
					task_suggestions: service.listTaskSuggestions({
						status: (url.searchParams.get("status") ?? undefined) as
							...
							| "rejected"
							| undefined,
						include_resolved: ["1", "true", "yes"].includes(
							(url.searchParams.get("all") ?? "").toLowerCase(),
						),
					}),
				});
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/task-suggestions"
			) {
				const body = await readJsonBody(request);
				sendJson(response, 200, {
					task_suggestion: service.createTaskSuggestion(
						extractIdentity(request, auth?.role ?? "operator"),
						{
							title: String(body.title ?? ""),
							notes: body.notes ? String(body.notes) : undefined,
							kind: String(body.kind ?? "assistant_work") as
								| "human_reminder"
								| "assistant_work",
							priority: String(body.priority ?? "normal") as
								| "low"
			...
			if (
				request.method === "GET" &&
				url.pathname.startsWith("/v1/task-suggestions/")
			) {
				const suggestionId = decodeURIComponent(
					url.pathname.slice("/v1/task-suggestions/".length),
				);
				if (suggestionId && !suggestionId.includes("/")) {
					sendJson(response, 200, {
						task_suggestion: service.getTaskSuggestionDetail(suggestionId),
					});
					return;
				}
			}
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/task-suggestions/prune"
			) {
				const body = await readJsonBody(request);
				sendJson(response, 200, {
					result: service.pruneTaskSuggestionHistory(
						...
						Array.isArray(body.statuses)
							? body.statuses.map(
									(value: unknown) => String(value) as "accepted" | "rejected",
								)
							: undefined,
					),
				});
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/planning-recommendations/refresh"
			) {
				sendJson(response, 200, {
					result: service.refreshPlanningRecommendations(
						extractIdentity(request, auth?.role ?? "operator"),
					),
				});
				return;
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/planning/autopilot"
			) {
				sendJson(response, 200, {
					planning_autopilot: await service.getPlanningAutopilotReport({
						httpReachable: true,
					}),
				});
				return;
			...
			if (
				...
				const bundleId = decodeURIComponent(
					url.pathname.slice("/v1/planning/autopilot/bundles/".length),
				);
				if (bundleId && !bundleId.includes("/")) {
					sendJson(response, 200, {
						planning_autopilot_bundle:
							await service.getPlanningAutopilotBundle(bundleId),
					});
					return;
				}
			}
			...
			if (
				...
				const bundleId = decodeURIComponent(
					url.pathname.slice(
						"/v1/planning/autopilot/bundles/".length,
						-"/prepare".length,
					),
				);
				if (bundleId && !bundleId.includes("/")) {
			...
			if (
				...
				if (bundleId && !bundleId.includes("/")) {
					...
					sendJson(response, 200, {
						planning_autopilot_bundle:
							await service.applyPlanningAutopilotBundle(
								extractIdentity(request, auth?.role ?? "operator"),
								bundleId,
								String(body.note ?? ""),
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/outbound/autopilot"
			) {
				sendJson(response, 200, {
					outbound_autopilot: await service.getOutboundAutopilotReport({
						httpReachable: true,
					}),
				});
				return;
			...
			if (
				...
				const groupId = decodeURIComponent(
					url.pathname.slice("/v1/outbound/autopilot/groups/".length),
				);
				if (groupId && !groupId.includes("/")) {
					sendJson(response, 200, {
						outbound_autopilot_group:
							await service.getOutboundAutopilotGroup(groupId),
					});
					return;
				}
			}
			...
			if (
				...
				const groupId = decodeURIComponent(
					url.pathname.slice(
						"/v1/outbound/autopilot/groups/".length,
						-"/request-approval".length,
					),
				);
				if (groupId && !groupId.includes("/")) {
					const body = await readJsonBody(request);
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/planning-recommendations"
			) {
				const grouped = ["1", "true", "yes"].includes(
					(url.searchParams.get("grouped") ?? "").toLowerCase(),
				);
				sendJson(response, 200, {
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/planning-recommendations/summary"
			) {
				sendJson(response, 200, {
					planning_recommendation_summary:
						service.getPlanningRecommendationSummaryReport(),
				});
				return;
			}
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/planning-recommendations/tuning"
			) {
				sendJson(response, 200, {
					planning_recommendation_tuning:
						service.getPlanningRecommendationTuningReport({
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/planning-recommendations/policy"
			) {
				sendJson(response, 200, {
					planning_recommendation_policy:
						service.getPlanningRecommendationPolicyReport(
							extractIdentity(request, auth?.role ?? "operator"),
						),
				});
				return;
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/planning-recommendations/backlog"
			) {
				const filters = {
					...
					resurfaced_only: parseBooleanQuery(
						url.searchParams.get("resurfaced_only"),
					),
				};
				sendJson(response, 200, {
					planning_recommendation_backlog:
						service.getPlanningRecommendationBacklogReport(filters),
				});
				return;
			}
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/planning-recommendations/closure"
			) {
				...
				sendJson(response, 200, {
					planning_recommendation_closure:
						service.getPlanningRecommendationClosureReport({
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/planning-recommendations/hygiene"
			) {
				sendJson(response, 200, {
					planning_recommendation_hygiene:
						service.getPlanningRecommendationHygieneReport(
							{
								group: url.searchParams.get("group") ?? undefined,
								kind: (url.searchParams.get("kind") ?? undefined) as any,
								source: (url.searchParams.get("source") ?? undefined) as any,
								candidate_only: parseBooleanQuery(
									url.searchParams.get("candidate_only"),
								),
								review_needed_only: parseBooleanQuery(
									url.searchParams.get("review_needed_only"),
								),
							},
							{ assistant_safe: auth?.role === "assistant" },
						),
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/planning-recommendations/hygiene/review"
			) {
				const body = await readJsonBody(request);
				sendJson(response, 200, {
					planning_recommendation_hygiene_family:
						service.reviewPlanningRecommendationHygiene(
							extractIdentity(request, auth?.role ?? "operator"),
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/planning-recommendations/hygiene/proposals/record"
			) {
				const body = await readJsonBody(request);
				sendJson(response, 200, {
					planning_recommendation_hygiene_family:
						service.recordPlanningRecommendationHygieneProposal(
							extractIdentity(request, auth?.role ?? "operator"),
			...
			if (
				...
				sendJson(response, 200, {
					planning_recommendation_hygiene_family:
						service.dismissPlanningRecommendationHygieneProposal(
							extractIdentity(request, auth?.role ?? "operator"),
							planningHygieneProposalInputFromBody(body),
						),
				});
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/planning-recommendations/policy/archive"
			) {
				const body = await readJsonBody(request);
				const identity = extractIdentity(request, auth?.role ?? "operator");
				const historyItem = service.archivePlanningRecommendationPolicy(
					identity,
					planningPolicyGovernanceInputFromBody(body),
				);
				sendJson(response, 200, {
					planning_recommendation_policy_history_item: historyItem,
					planning_recommendation_policy:
						service.getPlanningRecommendationPolicyReport(identity),
				});
				return;
			}
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/planning-recommendations/policy/supersede"
			) {
				const body = await readJsonBody(request);
				const identity = extractIdentity(request, auth?.role ?? "operator");
				const historyItem = service.supersedePlanningRecommendationPolicy(
					identity,
					planningPolicyGovernanceInputFromBody(body),
				);
				sendJson(response, 200, {
			...
			if (
				request.method === "POST" &&
				url.pathname === "/v1/planning-recommendations/policy/prune"
			) {
				const body = await readJsonBody(request);
				const identity = extractIdentity(request, auth?.role ?? "operator");
				sendJson(response, 200, {
					planning_recommendation_policy_prune:
						service.prunePlanningRecommendationPolicyHistory(
							identity,
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/planning-recommendation-groups"
			) {
				sendJson(response, 200, {
					planning_recommendation_groups:
						service.listPlanningRecommendationGroups({
							status: (url.searchParams.get("status") ?? undefined) as any,
							kind: (url.searchParams.get("kind") ?? undefined) as any,
							include_resolved: ["1", "true", "yes"].includes(
			...
			if (
				...
				const groupKey = decodeURIComponent(
					url.pathname
						.replace("/v1/planning-recommendation-groups/", "")
						.replace("/snooze", ""),
				);
				const body = await readJsonBody(request);
				sendJson(response, 200, {
					planning_recommendation_group:
						service.snoozePlanningRecommendationGroup(
							extractIdentity(request, auth?.role ?? "operator"),
							groupKey,
			...
			if (
				...
				const groupKey = decodeURIComponent(
					url.pathname.slice("/v1/planning-recommendation-groups/".length),
				);
				if (groupKey && !groupKey.includes("/")) {
					sendJson(response, 200, {
						planning_recommendation_group:
							service.getPlanningRecommendationGroupDetail(groupKey),
					});
					return;
				}
			}
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/planning-recommendations/next"
			) {
				const groupKey = url.searchParams.get("group") ?? undefined;
				sendJson(response, 200, {
					planning_recommendation: service.getNextPlanningRecommendationDetail(
						groupKey ?? undefined,
						{
							assistant_safe: auth?.role === "assistant",
						},
			...
			if (
				...
				const recommendationId = decodeURIComponent(
					url.pathname
						.replace("/v1/planning-recommendations/", "")
						.replace("/replan", ""),
				);
				const body = await readJsonBody(request);
				sendJson(response, 200, {
			...
			if (
				...
				const recommendationId = decodeURIComponent(
					url.pathname.slice("/v1/planning-recommendations/".length),
				);
				if (recommendationId && !recommendationId.includes("/")) {
					sendJson(response, 200, {
						planning_recommendation: service.getPlanningRecommendationDetail(
							recommendationId,
							{
								assistant_safe: auth?.role === "assistant",
							},
			...
			if (request.method === "GET" && url.pathname === "/v1/audit/events") {
				...
				const filter: AuditEventFilter = {
					limit: parseLimit(
						url.searchParams.get("limit") ?? String(policy.auditDefaultLimit),
					),
					category: parseAuditCategoryQuery(url.searchParams.get("category")),
				};
				sendJson(response, 200, {
					events: service.listAuditEvents(filter, {
						assistant_safe: auth?.role === "assistant",
					}),
				});
				return;
			...
			if (request.method === "GET" && url.pathname === "/v1/approval-queue") {
				const filter: ApprovalRequestFilter = {
					...
					state: (url.searchParams.get("state") ??
						undefined) as ApprovalRequestFilter["state"],
				};
				sendJson(response, 200, {
					approval_requests: service.listApprovalQueue(filter),
				});
				return;
			}
			...
			if (
				request.method === "GET" &&
				url.pathname === "/v1/approval-queue/pending"
			) {
				sendJson(response, 200, {
					approval_requests: service.listApprovalQueue({
						state: "pending",
						limit: policy.auditDefaultLimit,
					}),
				});
				return;
			}
			...
			if (
				request.method === "GET" &&
				url.pathname.startsWith("/v1/approval-queue/")
			) {
				const approvalId = decodeURIComponent(
					url.pathname.slice("/v1/approval-queue/".length),
				);
				if (approvalId && !approvalId.includes("/")) {
					sendJson(response, 200, {
						approval: service.getApprovalDetail(approvalId),
					});
					return;
				}
			}
				...
				sendJson(response, 200, {
					approval: service.approveRequest(
						...
						body.confirmation_token
							? String(body.confirmation_token)
							: undefined,
					),
				});
				return;
			...
			if (
				...
				const approvalId = decodeURIComponent(
					url.pathname.replace("/v1/approval-queue/", "").replace("/send", ""),
				);
				const body = await readJsonBody(request);
				sendJson(response, 200, {
			...
			if (request.method === "POST" && url.pathname === "/v1/snapshots") {
				...
				sendJson(response, 200, {
					snapshot: await service.createSnapshot(status.state),
				});
				return;
			}
			...
			if (
				request.method === "GET" &&
				url.pathname.startsWith("/v1/snapshots/")
			) {
				const snapshotId = decodeURIComponent(
					url.pathname.slice("/v1/snapshots/".length),
				);
				if (snapshotId && !snapshotId.includes("/")) {
					sendJson(response, 200, {
						snapshot: service.inspectSnapshot(snapshotId),
					});
					return;
				}
				return;
