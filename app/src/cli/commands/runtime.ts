...
async function runSafeExecutableStep(
	stepId: RepairStepId,
	paths: Paths,
): Promise<void> {
	if (stepId === "install_wrappers") {
	...
	if (stepId === "install_launchagent") {
		await installLaunchAgent(
			...
			process.execPath,
			{},
			{ trackRepairExecution: false },
		);
		return;
	}
	if (stepId === "install_desktop") {
		await installDesktop(paths, process.execPath, {
			trackRepairExecution: false,
		});
		return;
	}
...
async function buildCurrentStatusReport(
	context: CliContext,
): Promise<ServiceStatusReport> {
	const response = await context.requestJson<{ status: ServiceStatusReport }>(
		"GET",
		"/v1/status",
	);
...
async function buildCurrentMaintenanceSession(
	context: CliContext,
	paths: Paths,
): Promise<MaintenanceSessionPlan> {
	try {
		...
		return buildMaintenanceSessionPlan({
			...
			maintenance_decision_explanation:
				status.maintenance_decision_explanation ?? null,
			maintenance_repair_convergence:
				status.maintenance_repair_convergence ?? null,
...
async function executeTrackedSafeStep(input: {
	...
	const beforeReport = await buildCurrentHealthReport(
		input.context,
		input.paths,
	);
	const startedAt = new Date().toISOString();
	await runSafeExecutableStep(input.step.id, input.paths);
	const afterReport = await buildCurrentHealthReport(
		input.context,
		input.paths,
	);
	const remainingStep =
		afterReport.repair_plan.steps.find((step) => step.id === input.step.id) ??
		null;
...
export function registerRuntimeCommands(
	...
	program
		...
		.command("test")
		.description(
			"Show a macOS test notification to confirm local notifications work.",
		)
		.action(() => {
	...
	program
		.command("console")
		.description(
			"Open the local operator console with narrow browser-safe actions.",
		)
		.option(
			"--print-url",
			"Print the console launch URL instead of opening the browser",
		)
		.action(async (options) => {
			const response = await context.requestJson<{
				console_session: { launch_url: string };
	...
	const desktop = program
		.command("desktop")
		.description("Open or inspect the optional native desktop shell.");
	desktop
		.command("open")
	...
	desktop
		...
		.action(async (options) => {
			const desktopStatus = await getDesktopStatusReport(paths);
			const db = fs.existsSync(paths.databaseFile)
				? new PersonalOpsDb(paths.databaseFile)
				: null;
			const repairPlan = buildRepairPlan({
				install_check: buildInstallCheckReport(paths),
				desktop: desktopStatus,
				recent_repair_executions:
					db?.listRepairExecutions({ days: 30, limit: 100 }) ?? [],
			});
			db?.close();
			const payload = {
				desktop: {
			...
			context.printOutput(
				payload,
				(value) => formatDesktopStatus(value.desktop),
				options.json,
			);
		});
	...
	const repair = program
		.command("repair")
		.description("Read or run the narrow local repair plan.");
	repair
		.command("plan")
		...
		.action(async (options) => {
			...
			context.printOutput(
				...
				(value) => formatRepairPlanReport(value.repair_plan),
				options.json,
			);
		});
	...
	repair
		.command("run")
		.description(
			"Run one safe executable repair step or print the next manual step.",
		)
		...
		.action(async (stepId, options) => {
			...
			if (!targetStep.executable) {
				...
				if (!SAFE_EXECUTABLE_STEP_IDS.has(targetStep.id)) {
					throw new Error(
						`Repair step \`${targetStep.id}\` is not executable from the CLI.`,
					);
				}
				const {
					beforeReport,
				...
				if (resolvedTargetStep) {
					...
					try {
						...
						if (priorResolvedCount + 1 >= 2) {
							preventiveFollowUp =
								preventiveFollowUpForStep(targetStep.id) ?? undefined;
						}
				...
				execution = {
					...
					resolved_target_step: resolvedTargetStep,
					next_repair_step:
						afterReport.repair_plan.first_repair_step ?? undefined,
			...
			context.printOutput(
				...
				(value) => formatRepairExecutionResult(value.repair_execution),
				options.json,
			);
		});
	...
	const maintenance = program
		.command("maintenance")
		.description("Run calm-window maintenance one safe step at a time.");
	maintenance
		.command("session")
		.description(
			"Show the current maintenance session plan or explain why it is deferred.",
		)
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			...
			context.printOutput(
				...
				(value) => formatMaintenanceSessionPlan(value.maintenance_session),
				options.json,
			);
		});
	...
	maintenance
		.command("run")
		.description(
			"Run the next safe maintenance step when a calm maintenance window is eligible.",
		)
		.command("next")
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			...
			if (!session.eligible_now || session.steps.length === 0) {
				...
				try {
					...
					const afterSession = buildMaintenanceSessionPlan({
						...
						maintenance_defer_memory:
							afterStatus.maintenance_defer_memory ?? null,
						maintenance_confidence: afterStatus.maintenance_confidence ?? null,
						maintenance_operating_block:
							afterStatus.maintenance_operating_block ?? null,
						maintenance_decision_explanation:
							afterStatus.maintenance_decision_explanation ?? null,
						maintenance_repair_convergence:
							afterStatus.maintenance_repair_convergence ?? null,
						recent_repair_executions: currentRecentExecutions,
					});
					const sessionComplete =
						resolvedTargetStep &&
						!handedOffToRepair &&
						afterSession.steps.length === 0;
					const nextMaintenanceStep = !handedOffToRepair
						? (afterSession.steps[0] ?? null)
						: null;
					const message = !resolvedTargetStep
						? remainingStep
					...
					result = {
						...
						next_command: nextMaintenanceStep
							? MAINTENANCE_RUN_NEXT_COMMAND
							: undefined,
						next_repair_step:
							afterReport.repair_plan.first_repair_step ?? undefined,
					...
					const message =
						error instanceof Error ? error.message : "Maintenance step failed.";
					const db = new PersonalOpsDb(paths.databaseFile);
					try {
						db.createRepairExecution({
							step_id: targetStep.step_id,
	...
	program
		.command("version")
		.description(
			"Show the current personal-ops version and the official source-first upgrade path.",
		)
		.option("--json", "Print raw JSON")
		.action((options) => {
			...
			context.printOutput(
				response,
				(value) => formatVersionReport(value.version),
				options.json,
			);
		});
	...
	const autopilot = program
		.command("autopilot")
		.description("Inspect or warm the continuous autopilot coordinator.");
	autopilot
		.command("status")
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			const response = await context.requestJson<{ autopilot: unknown }>(
				"GET",
				"/v1/autopilot/status",
			);
			context.printOutput(
				response,
				(value) => formatAutopilotStatusReport(value.autopilot),
				options.json,
			);
		});
	...
	autopilot
		...
		.option(
			"--profile <profile>",
			"Run only one profile: day_start, inbox, meetings, planning, or outbound",
		)
		.option("--json", "Print raw JSON")
		.action(async (options) => {
	...
	program
		.command("status")
		.description(
			"Show the full operator readiness summary for the local personal-ops service.",
		)
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			const response = await context.requestJson<{ status: unknown }>(
				"GET",
				"/v1/status",
			);
			context.printOutput(
				response,
				(value) => formatStatusReport(value.status),
				options.json,
			);
		});
	...
	program
		...
		.action(async (options) => {
			...
			context.printOutput(
				payload,
				(value) => formatNowReport(value.status, value.worklist),
				options.json,
			);
		});
	...
	program
		...
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			const response = await context.requestJson<{ worklist: unknown }>(
				"GET",
				"/v1/worklist",
			);
			context.printOutput(
				response,
				(value) => formatWorklistReport(value.worklist),
				options.json,
			);
		});
	...
	const assistant = program
		.command("assistant")
		.description("Inspect or run the safe assistant action queue.");
	assistant
		.command("queue")
		.description(
			"Show the current assistant action queue with safe one-click actions and review-gated items.",
		)
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			const response = await context.requestJson<{ assistant_queue: unknown }>(
				"GET",
				"/v1/assistant/actions",
			);
			context.printOutput(
				response,
				(value) => formatAssistantQueueReport(value.assistant_queue),
				options.json,
			);
		});
	...
	assistant
		...
		.action(async (actionId, options) => {
			...
			context.printOutput(
				response,
				(value) => formatAssistantActionRunResult(value.assistant_run),
				options.json,
			);
		});
	...
	const github = program
		.command("github")
		.description("Read the narrow GitHub PR and review queue context.");
	github
		.command("status")
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			const response = await context.requestJson<{ github: unknown }>(
				"GET",
				"/v1/github/status",
			);
			context.printOutput(
				response,
				(value) => formatGithubStatus(value.github),
				options.json,
			);
		});
	...
	github
		...
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			const response = await context.requestJson<{ pull_requests: unknown[] }>(
				"GET",
				"/v1/github/reviews",
			);
			context.printOutput(
				response,
				(value) =>
					formatGithubPullRequests("GitHub Reviews", value.pull_requests),
				options.json,
			);
		});
	...
	github
		...
		.action(async (options) => {
			...
			context.printOutput(
				response,
				(value) =>
					formatGithubPullRequests("GitHub Pull Requests", value.pull_requests),
				options.json,
			);
		});
	...
	github
		...
		.action(async (prKey, options) => {
			...
			context.printOutput(
				response,
				(value) => formatGithubPullDetail(value.pull_request),
				options.json,
			);
		});
	...
	const drive = program
		.command("drive")
		.description("Read the narrow Google Drive and Docs context.");
	drive
		.command("status")
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			const response = await context.requestJson<{ drive: unknown }>(
				"GET",
				"/v1/drive/status",
			);
			context.printOutput(
				response,
				(value) => formatDriveStatus(value.drive),
				options.json,
			);
		});
	...
	drive
		...
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			const response = await context.requestJson<{ files: unknown[] }>(
				"GET",
				"/v1/drive/files",
			);
			context.printOutput(
				response,
				(value) => formatDriveFiles(value.files),
				options.json,
			);
		});
	...
	drive
		...
		.action(async (fileId, options) => {
			...
			context.printOutput(
				response,
				(value) => formatDriveSheet(value.sheet),
				options.json,
			);
		});
	...
	drive
		...
		.action(async (fileId, options) => {
			...
			context.printOutput(
				response,
				(value) => formatDriveDoc(value.doc),
				options.json,
			);
		});
	...
	const workflow = program
		.command("workflow")
		.description(
			"Compose the day-start operator flow into bounded workflow bundles.",
		);
	workflow
		.command("morning")
		.description(
			"Print the morning briefing: calendar, inbox, tasks, portfolio pulse, AI cost, alerts. " +
				"Archives to ~/Notes/personal-ops/YYYY-MM-DD.md.",
		)
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			type MorningResponse = {
				morning_briefing: Parameters<typeof formatMorningBriefing>[0];
			};
			const response = await context.requestJson<MorningResponse>(
				"GET",
				"/v1/workflows/morning",
			);
			const formatted = formatMorningBriefing(response.morning_briefing);
			if (options.json) {
				context.printOutput(response, () => formatted, true);
			} else {
				process.stdout.write(formatted + "\n");
				// Archive to Obsidian daily note
				try {
					const home = process.env["HOME"] ?? os.homedir();
					const notesDir = path.join(home, "Notes", "personal-ops");
					fs.mkdirSync(notesDir, { recursive: true });
					const dateStr = response.morning_briefing.date;
					...
					// Archive failure is non-fatal — briefing already printed
				}
			}
		});
	...
	workflow
		.command("now-next")
		.description(
			"Show the single best next move right now, plus a short backup list if that path is blocked.",
		)
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			const response = await context.requestJson<{ workflow: unknown }>(
				"GET",
				"/v1/workflows/now-next",
			);
			context.printOutput(
				response,
	...
	workflow
		.command("prep-day")
		.description(
			"Show the preferred day-start bundle with top attention, time-sensitive items, and exact next commands.",
		)
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			const response = await context.requestJson<{ workflow: unknown }>(
				"GET",
				"/v1/workflows/prep-day",
			);
			context.printOutput(
				response,
				(value) => formatWorkflowBundleReport(value.workflow),
	...
	workflow
		...
		.description(
			"Show the bounded follow-up bundle for reply pressure, stale nudges, and next commands.",
		)
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			const response = await context.requestJson<{ workflow: unknown }>(
				"GET",
				"/v1/workflows/follow-up-block",
			);
			context.printOutput(
				response,
				(value) => formatWorkflowBundleReport(value.workflow),
	...
	workflow
		...
		.option(
			"--event <eventId>",
			"Show the full packet detail for one calendar event",
		)
		.option(
			"--prepare",
			"Prepare or refresh the packet for the selected event before showing it",
		)
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			...
			if (options.event) {
				...
				if (options.prepare) {
					const prepared = await context.requestJson<{
						meeting_prep_packet: { packet: unknown };
					}>(
						"POST",
					...
					context.printOutput(
						prepared,
						(value) =>
							formatMeetingPrepPacket(value.meeting_prep_packet.packet),
						options.json,
					);
					return;
				}
				const response = await context.requestJson<{
					meeting_prep_packet: unknown;
				}>("GET", `/v1/workflows/prep-meetings/${encodedEventId}`);
				context.printOutput(
					response,
					(value) => formatMeetingPrepPacket(value.meeting_prep_packet),
					options.json,
				);
				return;
			}
	...
	program
		...
		.option(
			"--deep",
			"Run a live Gmail verification call in addition to local checks",
		)
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			const query = options.deep ? "?deep=true" : "";
			const response = await context.requestJson<{ doctor: unknown }>(
				"GET",
				`/v1/doctor${query}`,
			);
			context.printOutput(
				response,
				(value) => formatDoctorReport(value.doctor),
				options.json,
			);
		});
	...
	const health = program
		.command("health")
		.description("Run recurring-friendly local health checks.");
	health
		.command("check")
		.description(
			"Run a compact health pass that combines install, runtime, and snapshot freshness.",
		)
	...
	const sendWindow = program
		.command("send-window")
		.description("Inspect or control the timed send window.");
	sendWindow
		.command("status")
		.option("--json", "Print raw JSON")
		.action(async (options) => {
			const response = await context.requestJson<{ send_window: unknown }>(
				"GET",
				"/v1/send-window",
			);
			context.printOutput(
				response,
				(value) => formatSendWindowStatus(value.send_window),
				options.json,
			);
		});
